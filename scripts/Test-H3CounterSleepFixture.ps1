...
param(
    ...
    [string] $FixturePath = (Join-Path $PSScriptRoot '..\tests\fixtures\h3\counter-sleep-v1.json'),
    [string] $SchemaPath = (Join-Path $PSScriptRoot '..\schemas\h3-counter-sleep-fixture.schema.json'),
...
if (-not ($fixtureJson | Test-Json -SchemaFile $SchemaPath)) { throw 'H3 counter-sleep fixture failed schema validation.' }
$fixture = $fixtureJson | ConvertFrom-Json
& (Join-Path $PSScriptRoot 'Test-RomBaseline.ps1') -RomPath $RomPath
if ((Get-FileHash -LiteralPath $RomPath -Algorithm SHA256).Hash -ne [string] $fixture.romSha256) { throw 'H3 counter-sleep ROM mismatch.' }
if ([string] $toolchain.bizhawk.release -ne [string] $fixture.emulator.version -or
    [string] $toolchain.bizhawk.core -ne [string] $fixture.emulator.core) { throw 'H3 counter-sleep emulator contract mismatch.' }
...
if ($damage -ne [int] $fixture.expected.firstDamage -or
    ...
    throw 'Static counter-sleep damage model mismatch.'
}

$observedJson = (& (Join-Path $PSScriptRoot 'Observe-H3Battle01TurnOrder.ps1') -RomPath $RomPath -Scenario counter-sleep -TimeoutSeconds $TimeoutSeconds) -join [Environment]::NewLine
...
if ($observed.system -ne 'GEN' -or $observed.core -ne [string] $fixture.emulator.core -or
    ...
    throw 'H3 counter-sleep execution context mismatch.'
}
foreach ($field in @('targetDies', 'status', 'counterBefore', 'counterAfter', 'damageCalls', 'targetHp')) {
    if ([string] $observed.$field -ne [string] $fixture.expected.$field) {
        throw "H3 counter-sleep mismatch: $field."
    }
}
...
[pscustomobject]@{
    ...
    StatusWord = ('0x{0:X4}' -f [int] $observed.status)
