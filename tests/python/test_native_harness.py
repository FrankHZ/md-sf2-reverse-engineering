...
def test_research_index_validates_without_private_inputs() -> None:
    ...
    assert result["Records"] == 1615
    assert result["Confirmed"] == 1615
    assert result["H2Fixtures"] == 74
    assert result["H3Fixtures"] == result["H3FixtureFiles"] == 78
    assert result["AddressBindings"] == 2428
    assert result["IndexedCodeFiles"] == 381
    assert result["IndexedDataFiles"] == 1017
    assert result["H1ListingRecords"] == 1578
