...
from sf2tool.h2.map3_battle01_action_completion import (
    verify_map3_battle01_action_completion_static,
)
...
from sf2tool.h2.map_event_flag_route_selection import (
    verify_map_event_flag_route_selection_contract,
)
...
from sf2tool.h2.map_event_random_battle_state import (
    verify_map_event_random_battle_state_contract,
)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    h2_field_item_effects = h2_commands.add_parser(
    ...
    h2_map3_castle_battle_unlock = h2_commands.add_parser(
    ...
    h2_map3_battle01_action_effect = h2_commands.add_parser(
        "map3-battle01-action-effect",
        help="verify the static Battle 01 ApplyActionEffect-to-DropEnemyItem contract",
    )
    _add_local_paths(h2_map3_battle01_action_effect)
    h2_map3_battle01_action_completion = h2_commands.add_parser(
    ...
    h2_map_event_flag_lifecycle_state = h2_commands.add_parser(
        "map-event-flag-lifecycle-state",
        help="verify same-program direct check/set/clear flag lifecycle relations",
    )
    ...
    h2_map_event_flag_route_selection = h2_commands.add_parser(
        "map-event-flag-route-selection",
        help="classify static cross-program flag candidates by map-event route topology",
    )
    _add_local_paths(h2_map_event_flag_route_selection)
    h2_map_event_flag_route_selection.add_argument("--output-path", type=_path)
    ...
    _add_local_paths(h2_map_event_request_state)
    h2_map_event_request_state.add_argument("--output-path", type=_path)
    h2_map_event_request_consumption = h2_commands.add_parser(
        "map-event-request-consumption",
        help="verify bounded static consumers of map-event request state",
    )
    _add_local_paths(h2_map_event_request_consumption)
    h2_map_event_request_consumption.add_argument("--output-path", type=_path)
    h2_map_event_dialogue_state = h2_commands.add_parser(
        "map-event-dialogue-state",
        help="verify bounded map-event dialogue state flow, text sites, and reaching definitions",
    )
    _add_local_paths(h2_map_event_dialogue_state)
    h2_map_event_dialogue_state.add_argument("--output-path", type=_path)
    h2_map_event_direct_control = h2_commands.add_parser(
        "map-event-direct-control",
        help="verify caller-side direct-transfer topology in map-event programs",
    )
    _add_local_paths(h2_map_event_direct_control)
    h2_map_event_direct_control.add_argument("--output-path", type=_path)
    h2_map_event_direct_handoff = h2_commands.add_parser(
        "map-event-direct-handoff",
        help="verify immediate setup and returning-call lexical handoff operations",
    )
    _add_local_paths(h2_map_event_direct_handoff)
    h2_map_event_direct_handoff.add_argument("--output-path", type=_path)
    h2_map_event_predicate_results = h2_commands.add_parser(
        "map-event-predicate-results",
        help="verify retained map-event predicate result source/H1/ROM branch facts",
    )
    _add_local_paths(h2_map_event_predicate_results)
    h2_map_event_predicate_results.add_argument("--output-path", type=_path)
    h2_map_event_random_battle_state = h2_commands.add_parser(
        "map-event-random-battle-state",
        help="verify static CheckRandomBattle caller and request-state structure",
    )
    _add_local_paths(h2_map_event_random_battle_state)
    h2_map_event_random_battle_state.add_argument("--output-path", type=_path)
    h2_map_event_tactical_base_quote_state = h2_commands.add_parser(
        "map-event-tactical-base-quote-state",
        help="verify static DisplayTacticalBaseQuote callers and source branch shape",
    )
    ...
    h3_church_cure = h3_commands.add_parser(
        "church-cure-lifecycle",
        help="observe Church Cure poison, stun, and curse transaction lifecycle",
    )
    ...
    _add_local_paths(h3_controller_input)
    h3_controller_input.add_argument("--timeout-seconds", type=int, default=180)
    ...
    _add_local_paths(h3_map3_messenger_acceptance)
    h3_map3_messenger_acceptance.add_argument("--timeout-seconds", type=int, default=300)
    ...
    h3_map_entity_placement = h3_commands.add_parser(
        "map-entity-placement",
        help="verify one-launch entity placement, facing, flash, destination, and wait boundaries",
    )
    ...
    h3_map_script_entity_presentation_fx = h3_commands.add_parser(
        "map-script-entity-presentation-fx",
        help=(
            "verify one-launch entity-presentation FX handler branch, loop, "
    ...
    _add_local_paths(h3_map_script_dialogue)
    h3_map_script_dialogue.add_argument("--timeout-seconds", type=int, default=180)
...
def dispatch(args: argparse.Namespace) -> None:
    ...
    elif args.command == "h2" and args.h2_command == "map3-battle01-action-effect":
        print_record(verify_map3_battle01_action_effect_static(args.rom_path, args.upstream_path))
    ...
    elif args.command == "h2" and args.h2_command == "map-event-flag-route-selection":
        print_record(
            verify_map_event_flag_route_selection_contract(
                args.rom_path,
                args.upstream_path,
    ...
    elif args.command == "h2" and args.h2_command == "map-event-request-state":
        print_record(
            verify_map_event_request_state_contract(
                args.rom_path,
                args.upstream_path,
                output_path=args.output_path,
            )
        )
    elif args.command == "h2" and args.h2_command == "map-event-request-consumption":
        print_record(
            verify_map_event_request_consumption_contract(
                args.rom_path,
                args.upstream_path,
    ...
    elif args.command == "h2" and args.h2_command == "map-event-dialogue-state":
        print_record(
            verify_map_event_dialogue_state_contract(
                args.rom_path,
                args.upstream_path,
                output_path=args.output_path,
            )
        )
    elif args.command == "h2" and args.h2_command == "map-event-direct-control":
        print_record(
            verify_map_event_direct_control_contract(
                args.rom_path,
                args.upstream_path,
                output_path=args.output_path,
            )
        )
    elif args.command == "h2" and args.h2_command == "map-event-direct-handoff":
        print_record(
            verify_map_event_direct_handoff_contract(
                args.rom_path,
                args.upstream_path,
                output_path=args.output_path,
            )
        )
    elif args.command == "h2" and args.h2_command == "map-event-predicate-results":
        print_record(
            verify_map_event_predicate_results_contract(
                args.rom_path,
                args.upstream_path,
    ...
    elif args.command == "h2" and args.h2_command == "map-event-random-battle-state":
        print_record(
            verify_map_event_random_battle_state_contract(
                args.rom_path,
                args.upstream_path,
    ...
    elif args.command == "h3" and args.h3_command == "controller-input":
        print_record(
            verify_controller_input(
                args.rom_path,
                args.upstream_path,
                timeout_seconds=args.timeout_seconds,
            )
    ...
    elif args.command == "h3" and args.h3_command == "map3-messenger-acceptance":
        print_record(
    ...
    elif args.command == "h3" and args.h3_command == "map-script-dialogue":
        print_record(
            verify_map_script_dialogue(
                args.rom_path,
                args.upstream_path,
                timeout_seconds=args.timeout_seconds,
            )
