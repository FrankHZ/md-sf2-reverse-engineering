...
REFERENCES = {
    ...
    "tests/fixtures/h3/physical-damage-application-v1.json": "sf2-physical-damage-application-v1",
    ...
    "tests/fixtures/h3/counter-burst-rock-v1.json": "sf2-counter-burst-rock-validation-v1",
...
def verify_design_contracts() -> dict[str, Any]:
    document_relative = "docs/design/combat-resolution.md"
    document_path = repo_path(document_relative)
    docs_index_path = repo_path("docs/README.md")
    if not document_path.is_file():
        raise ValueError(f"missing design contract: {document_relative}")
    document = document_path.read_text(encoding="utf-8")
    docs_index = docs_index_path.read_text(encoding="utf-8")
    if "./design/combat-resolution.md" not in docs_index:
        raise ValueError("docs/README.md does not index the physical combat contract")
    for fixture_relative, fixture_id in REFERENCES.items():
        ...
        if fixture_relative not in document or fixture_id not in document:
            raise ValueError(f"design contract does not trace {fixture_id} to {fixture_relative}")
    if "**Confirmed" not in document or "**Unknown" not in document:
        raise ValueError("design contract must preserve Confirmed and Unknown evidence labels")
    return {
        "Document": document_relative,
        "FixtureReferences": len(REFERENCES),
