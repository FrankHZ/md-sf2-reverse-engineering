...
from sf2tool.remake_asset_build import (
    ACCEPTED_ROM_SHA256, ACCEPTED_ROM_SIZE, ACCEPTED_UPSTREAM_COMMIT,
    ACCEPTED_UPSTREAM_REPOSITORY, _composite_generator_fingerprint,
    _scale_rgba_nearest,
)
from sf2tool.remake_assets import (
    MANIFEST_RELATIVE_PATH, PACK_CAPABILITY, PACK_SCHEMA, PACKAGE_ID, PROFILE,
    REPOSITORY_ID,
)
from sf2tool.texture_extract import decode_md_4bpp_tile, md_palette_color, palette_index_rgba, write_png_rgba
...
def compose(data: bytes, palette: list, width: int, height: int, layout: list[int], *, opaque=False) -> list[int]:
    ...
    for cell, word in enumerate(layout):
        ...
        for y in range(8):
            for x in range(8):
                ...
                offset = (((cell // (width // 8)) * 8 + y) * width + (cell % (width // 8)) * 8 + x) * 4
...
def sprite_layout(columns: int, rows: int) -> list[int]:
    ...
    return [((x//4)*((rows+3)//4)+(y//4))*16+(x%4)*4+y%4 for y in range(rows) for x in range(columns)]
...
def build(rom_path: Path, upstream: Path, output: Path) -> dict:
    ...
    def read(address: int, count: int) -> bytes:
        ...
        spans.append({"address": address, "byteLength": count, "data": base64.b64encode(data).decode()})
        return data

    def word(address): return int.from_bytes(read(address, 2), "big")
    def pointer(address): return int.from_bytes(read(address, 4), "big")
    ...
    def palette(address): return [md_palette_color(word(address+i*2) & 0x0EEE) for i in range(16)]
    ...
    def table(name):
        return json.loads(repo_path("tests/fixtures/h2/"+name+"-v1.json").read_text(encoding="utf-8"))["table"]

    components = {name: repo_path(name).read_bytes() for name in (
        "src/sf2tool/remake_battle_scene_content.py", "src/sf2tool/remake_asset_build.py",
        "src/sf2tool/texture_extract.py", "src/sf2tool/compression.py")}
    ...
    def raster(name, data, pal, width, height, layout, *, opaque=False):
        ...
        for scale in (2, 4):
            ...
            write_png_rgba(target, width*scale, height*scale, _scale_rgba_nearest(tuple(pixels), width, height, scale))
            encoded = target.read_bytes()
            buckets.append(dict(scale=scale, runtimePath=relative, width=width*scale, height=height*scale,
                byteLength=len(encoded), sha256=digest(encoded), mediaType="image/png", filter="nearest",
                mipmaps=False, repeat=False, colorSpace="srgb", alphaMode="straight"))
            if scale == 2:
                rasters[name] = dict(width=width*2, height=height*2, format="png", data=base64.b64encode(encoded).decode(), sha256=digest(encoded))
        assets.append(dict(assetId="battle.scene."+name.replace("/", "."), kind="raster-image",
            logicalSize=dict(width=width, height=height),
            source=dict(assetId="source.battle.scene.selection", sha256=""),
            derivation=dict(policyId="source-layout-nearest-v1", generatorId="sf2tool-remake-battle-scene-content",
                generatorVersion="1", generatorArtifactSha256=fingerprint), buckets=buckets))
        return name
    ...
    actor_rows = []
    for side, index, pal_index, weapon_item in (("ally",0,0,71),("ally",1,0,85),("ally",2,0,72),("enemy",22,2,None)):
        ...
        for frame in range((pal_offset-base-6)//2):
            ...
            frames.append(raster(f"{side}{index}/frame{frame}", data, pal, 96 if side=="ally" else 128, 96,
                sprite_layout(12 if side=="ally" else 16, 12)))
        sequences = {}
        for purpose, animation in (("attack",80 if weapon_item==72 else index),("dodge",index+(40 if side=="ally" else 60)),("idle",index)):
            ...
            def signed(value): return value if value<128 else value-256
            def weapon(raw): return dict(frame=raw[0], layer=raw[1], x=signed(raw[2]), y=signed(raw[3]))
            rows = []
            for offset in range(size, len(data), size):
                raw = data[offset:offset+size]
                rows.append(dict(frame=raw[0], ticks=raw[1], x=signed(raw[2]), y=signed(raw[3]),
                    weapon=weapon(raw[4:]) if side=="ally" else None))
            sequences[purpose] = dict(index=animation, trigger=data[1], spell=data[2], terminate=data[3],
                idleWeapon=weapon(data[4:8]) if side=="ally" else None, frames=rows)
        weapon_frames = []
        if weapon_item is not None:
            ...
            weapon_pal = list(pal)
            weapon_pal[14:16] = [md_palette_color(word(weapons["weaponPaletteAddress"]+pid*4+i*2)) for i in range(2)]
            data = decode(pointer(weapons["weaponSpriteTableAddress"]+wid*4), 8192)
            for frame in range(4):
                weapon_frames.append(raster(f"weapon{weapon_item}/frame{frame}", data[frame*2048:(frame+1)*2048],
                    weapon_pal,64,64,sprite_layout(8,8)))
        actor_rows.append(dict(side=side, sprite=index, palette=pal_index, item=weapon_item,
            frames=frames, weaponFrames=weapon_frames, sequences=sequences))
    ...
    ground_asset = raster("ground9", decode(ground+6+word(ground+6),1536),base_palette,96,32,sprite_layout(12,4))
    backgrounds = table("battle-background-decode")
    background = pointer(backgrounds["backgroundTableAddress"]+9*4)
    bgdata = decode(background+word(background),6144)+decode(background+2+word(background+2),6144)
    ...
    def source_text(relative):
        return subprocess.check_output(["git", "-C", str(upstream), "show", commit+":"+relative]).decode("utf-8")
    script = source_text("disasm/data/scripting/text/gamescript.txt")
    text_ids = {244,263,266,267,268,269,270,271,273,284,285,286,287,288,290,291,292,293,393}
    texts = {str(int(line[:4],16)):line[5:] for line in script.splitlines()
    ...
    bundle = dict(romSha256=ACCEPTED_ROM_SHA256, upstreamRepository=ACCEPTED_UPSTREAM_REPOSITORY,
        upstreamCommit=commit, spans=spans, texts=texts, memberNames=names,
        textSource="disasm/data/scripting/text/gamescript.txt",nameSource="disasm/data/stats/allies/allynames.asm")
    bundle_path = output / "source/battle-scenes/selection.json"
    write_json(bundle_path,bundle)
    source_hash = digest(bundle_path.read_bytes())
    for asset in assets: asset["source"]["sha256"] = source_hash
    manifest = dict(schemaVersion=1,packageId=PACKAGE_ID,repositoryId=REPOSITORY_ID,profile=PROFILE,
        capabilities=[PACK_CAPABILITY],logicalPresentation=dict(width=960,height=540),assets=assets)
    ...
    write_json(output / "battle-scenes.json",dict(version=1,encounter="battle-1",background=background_asset,
        ground=ground_asset,actors=actor_rows,rasters=rasters,texts=texts,memberNames=names))
    report = dict(status="candidate-only",upstreamCommit=commit,romSha256=ACCEPTED_ROM_SHA256,
        generatorArtifactSha256=fingerprint,manifestSha256=digest((output/MANIFEST_RELATIVE_PATH).read_bytes()),
        sourceSha256=source_hash,assetCount=len(assets),sceneContentBytes=(output/"battle-scenes.json").stat().st_size,
