...
local construction_records, award, exp_reaction = {}, nil, nil
local ally_reactions, enemy_reactions, reaction_order = {}, {}, {}
local active_construction, active_ally, active_enemy = nil, nil, nil
local construction_actor_mp, construction_actor_status = nil, nil
...
local function write_reactions(out, reactions)
    for i,r in ipairs(reactions) do
        if i>1 then out:write(",") end
        out:write(string.format(
            '{"combatant":%d,"mpChange":%d,"mpBefore":%d,"mpAfter":%d}',
            r.combatant,r.mpChange,r.mpBefore,r.mpAfter))
    end
end
...
local function write_result_and_exit()
    ...
    out:write(string.format(
        ...
        config.case.targets[1].combatant))
    for i,r in ipairs(construction_records) do
        if i>1 then out:write(",") end
        out:write(string.format(
            '{"combatant":%d,"randomRoll":%d,"unclampedTransfer":%d,"targetMp":%d,"transfer":%d,"accumulatedExp":%d}',
            r.combatant,r.randomRoll,r.unclampedTransfer,r.targetMp,r.transfer,r.accumulatedExp))
    end
    out:write(string.format(
        '],"accumulatedExp":%d,"actorMp":%d,"actorStatus":%d,'..
        '"award":{"seed":%d,"halved":%d,"firstRoll":%d,"secondRoll":%d,"commandExp":%d}},'..
        '"replay":{"reactionOrder":[',
        construction_records[#construction_records].accumulatedExp,construction_actor_mp,construction_actor_status,
    ...
    out:write('],"allyReactions":[');write_reactions(out,ally_reactions)
    out:write('],"enemyReactions":[');write_reactions(out,enemy_reactions)
    out:write(string.format(
        '],"expReaction":{"commandExp":%d,"expBefore":%d,"expAfter":%d},'..
        '"finalActorMp":%d,"finalActorExp":%d,"finalActorStatus":%d,"finalTargetMp":[',
    ...
    for i,target in ipairs(config.case.targets) do
        if i>1 then out:write(",") end
        out:write(tostring(memory.read_u8(entry(target.combatant)+17,"M68K BUS")))
    end
...
event.on_bus_exec(function() prompt_count=prompt_count+1;bootstrap.battle01_intro_skip(config.bootstrap.profile,prompt_count,pulse) end,config.harness["function"].numberPromptAddress,"sf2-mp-number","M68K BUS")
event.on_bus_exec(function() pulse("B") end,config.harness["function"].flagPromptAddress,"sf2-mp-flag","M68K BUS")
event.on_bus_exec(function()
    stage="battle";local a=entry(config.case.actor)
    ...
    for i,target in ipairs(config.case.targets) do
        ...
        memory.write_u8(t+16,target.maxMp,"M68K BUS");memory.write_u8(t+17,target.initialMp,"M68K BUS")
        memory.write_u16_be(t+26,0,"M68K BUS");memory.write_u16_be(t+28,0,"M68K BUS");memory.write_u16_be(t+44,0,"M68K BUS")
        memory.write_u8(t+46,8+i-1,"M68K BUS");memory.write_u8(t+47,17,"M68K BUS");memory.write_u8(t+49,0x60,"M68K BUS");memory.write_u8(t+55,0,"M68K BUS")
...
event.on_bus_exec(function()
    ...
    active_construction={combatant=combatant,targetMp=memory.read_u8(entry(combatant)+17,"M68K BUS")}
end,config["function"].absorbEffectEntryAddress,"sf2-mp-effect","M68K BUS")
event.on_bus_exec(function() if active_construction then active_construction.randomRoll=word("M68K D0");active_construction.unclampedTransfer=active_construction.randomRoll+3 end end,config["function"].randomRollAddress,"sf2-mp-roll","M68K BUS")
event.on_bus_exec(function() if active_construction then active_construction.transfer=word("M68K D0") end end,config["function"].clampedTransferAddress,"sf2-mp-clamp","M68K BUS")
event.on_bus_exec(function()
    if active_construction then
        active_construction.accumulatedExp=memory.read_u16_be(config.ram.battleSceneExpAddress,"M68K BUS")
        construction_records[#construction_records+1]=active_construction;active_construction=nil
    end
end,config["function"].statusExpAppliedAddress,"sf2-mp-exp","M68K BUS")

event.on_bus_exec(function() if not action_started then return end;award={seed=memory.read_u16_be(config.ram.seedAddress,"M68K BUS"),halved=word("M68K D1")} end,config["function"].expHalvedAddress,"sf2-mp-half","M68K BUS")
...
event.on_bus_exec(function()
    if #construction_records==#config.case.targets then
        local actor=entry(config.case.actor);construction_actor_mp=memory.read_u8(actor+17,"M68K BUS")
        construction_actor_status=memory.read_u16_be(actor+44,"M68K BUS");playback=true
    end
end,config["function"].battleSceneEndReturnAddress,"sf2-mp-end","M68K BUS")

event.on_bus_exec(function()
    if not playback then return end;local a6=emu.getregister("M68K A6")&0xFFFFFF
    active_ally={combatant=config.case.actor,mpChange=signed(memory.read_u16_be(a6+2,"M68K BUS")),mpBefore=memory.read_u8(entry(config.case.actor)+17,"M68K BUS")}
...
event.on_bus_exec(function() if active_ally then active_ally.mpAfter=memory.read_u8(entry(config.case.actor)+17,"M68K BUS");ally_reactions[#ally_reactions+1]=active_ally;active_ally=nil end end,config["function"].casterMpAppliedAddress,"sf2-mp-ally-applied","M68K BUS")
event.on_bus_exec(function()
    if not playback then return end;local a6=emu.getregister("M68K A6")&0xFFFFFF
    local combatant=memory.read_u16_be(config.ram.battleSceneEnemyAddress,"M68K BUS")
    active_enemy={combatant=combatant,mpChange=signed(memory.read_u16_be(a6+2,"M68K BUS")),mpBefore=memory.read_u8(entry(combatant)+17,"M68K BUS")}
    reaction_order[#reaction_order+1]="enemy:"..tostring(active_enemy.mpChange)
end,config["function"].enemyReactionEntryAddress,"sf2-mp-enemy","M68K BUS")
event.on_bus_exec(function() if active_enemy then active_enemy.mpAfter=memory.read_u8(entry(active_enemy.combatant)+17,"M68K BUS");enemy_reactions[#enemy_reactions+1]=active_enemy;active_enemy=nil end end,config["function"].enemyMpAppliedAddress,"sf2-mp-enemy-applied","M68K BUS")
...
event.on_bus_exec(function() if playback and #ally_reactions==4 and #enemy_reactions==3 and exp_reaction and exp_reaction.expAfter then write_result_and_exit() end end,config["function"].executeScriptEndAddress,"sf2-mp-script-end","M68K BUS")
...
while true do
    ...
    if frames%600==0 then status(string.format("frame=%d,stage=%s,pc=%X,records=%d,allies=%d,enemies=%d",frames,stage,emu.getregister("M68K PC"),#construction_records,#ally_reactions,#enemy_reactions)) end
end
