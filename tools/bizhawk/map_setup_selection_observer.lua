...
local function finish()
    ...
    for i,r in ipairs(records) do
        ...
        f:write(string.format('{"id":"%s","map":%d,"selectedAddress":%d}',r.id,r.map,r.selectedAddress))
    end
...
end,config.harness["function"].numberPromptAddress,"sf2-map-select-number","M68K BUS")
event.on_bus_exec(function() status("milestone:number-prompt-return");end,config.navigation.mapPromptReturnAddress,"sf2-map-select-number-return","M68K BUS")
event.on_bus_exec(function() status("milestone:flag-prompt-entry");pulse("B") end,config.harness["function"].flagPromptAddress,"sf2-map-select-flag","M68K BUS")
event.on_bus_exec(function() status("milestone:flag-prompt-return");end,config.navigation.flagPromptReturnAddress,"sf2-map-select-flag-return","M68K BUS")
event.on_bus_exec(function() status("milestone:exploration-loop") end,config.navigation.explorationLoopAddress,"sf2-map-select-exploration","M68K BUS")
event.on_bus_exec(function() status("milestone:get-entity-list") end,config.navigation.getMapSetupEntityListAddress,"sf2-map-select-entities","M68K BUS")
event.on_bus_exec(function() status("milestone:run-init") end,config.navigation.runMapSetupInitFunctionAddress,"sf2-map-select-init","M68K BUS")

event.on_bus_exec(function()
    if active then return end
    local case=config.cases[case_index];setup_case(case);active=true;status("milestone:case:"..case.id)
end,config["function"].entryAddress,"sf2-map-select-entry","M68K BUS")

event.on_bus_exec(function()
    if not active then return end
    local case=config.cases[case_index]
    records[#records+1]={id=case.id,map=case.map,selectedAddress=emu.getregister("M68K A0")&0xFFFFFF}
    active=false;case_index=case_index+1
    if case_index>#config.cases then finish() else pending_replay=true end
end,config["function"].returnAddress,"sf2-map-select-return","M68K BUS")
