...
-- fulfillment cohort jumps to original @AddItem; the pre-commit and
-- prompt-routing cohorts enter BlacksmithAction_FulfillOrder at its
-- selection-loop label; and the retained optional-equip cohort runs last.
-- Harness-only service stubs return through original source branches and stop
-- before any excluded presentation body.
...
local probe_base,helper_base,helper_stride,transaction_base,transaction_stride,fulfillment_base,fulfillment_stride,precommit_base,precommit_stride,equip_base,equip_stride=0xFF6800,0xFF6820,24,0xFF6900,32,0xFF6960,32,0xFF6B00,32,0xFF6C20,32
routing_base,routing_stride=0xFF6BA0,32
...
rti={promptBoundaryReadbacks={},abortPresentationSkipReadback=false,generatedPromptStubWriteReadback=false,doneRedirectReadback=false,jointDispatcherWriteReadback=false,generatedDoneMovemStubWriteReadback=false,generatedDoneRtsStubWriteReadback=false,doneMovemStackReadback=false,doneRtsStackReadback=false}
...
local ed={active=false,pending=nil,jointDispatcherPending=false,frameCount=0,frameBudget=180,transition={active=false,frameCount=0,frameBudget=180},record=nil,chronology=nil,expectedTerminal="none",terminal="none",completedIndex=0,completedTerminal="none",completedExpectedTerminal="none"}
...
rtx={active=false,attemptIndex=0,memberListCallCount=0,heldItemsCallCount=0,equipmentTypeCallCount=0,equippabilityCallCount=0,pendingService=nil,promptBoundaryReturnAddress=nil,terminalStubPurpose=nil,controlledPrompt=nil,jointDispatcherPending=false,doneMovemPending=false,doneRtsPending=false,dispatchStackDiagnostic=nil,selectedMembers=nil,routeTerminal="none",completionTerminal="none",frameCount=0,frameBudget=0,record=nil,chronology=nil,cleanup={active=false,case=nil,addItemReturnSeen=false,orderReadSeen=false,orderClearedSeen=false,fulfilledOrdersIncrementSeen=false,equippabilityCarrySet=nil}}
...
function current_routing() return config.promptRoutingCases[routing_index] end
...
function routing_pc(index) return routing_base+(index-1)*routing_stride end
...
function routing_state_json()
  local pending="null";if rtx.pendingService~=nil then local service=rtx.pendingService;pending="{\"callPc\":"..service.callPc..",\"returnPc\":"..service.returnPc..",\"role\":"..json_string(service.role)..",\"targetPc\":"..service.targetPc.."}" end
  return "{\"active\":"..bool(rtx.active)..",\"attemptIndex\":"..rtx.attemptIndex..",\"equipmentTypeCallCount\":"..rtx.equipmentTypeCallCount..",\"equippabilityCallCount\":"..rtx.equippabilityCallCount..",\"frameBudget\":"..rtx.frameBudget..",\"frameCount\":"..rtx.frameCount..",\"heldItemsCallCount\":"..rtx.heldItemsCallCount..",\"memberListCallCount\":"..rtx.memberListCallCount..",\"mode\":"..json_string(mode)..",\"pendingService\":"..pending..",\"routeTerminal\":"..json_string(rtx.routeTerminal)..",\"completionTerminal\":"..json_string(rtx.completionTerminal).."}"
end
...
function pending_callback_state()
  ...
  if mode=="helper" or current_role=="case-entry" then case_for_state=helper_index elseif mode=="transaction" or current_role=="transaction-case-entry" then case_for_state=transaction_index elseif mode=="fulfillment" or current_role=="fulfillment-case-entry" then case_for_state=fulfillment_index elseif mode=="precommit" or mode=="precommit-cleanup" or current_role=="precommit-case-entry" or pcx.transition.active then case_for_state=precommit_index elseif mode=="prompt-routing" or mode=="prompt-routing-cleanup" or current_role=="prompt-routing-case-entry" then case_for_state=routing_index elseif mode=="equip-decision" or current_role=="equip-decision-case-entry" or current_role=="equip-decision-transition" or current_role=="equip-decision-transition-timeout" or ed.transition.active then case_for_state=equip_index end
  return "{\"active\":"..bool(helper_active or tx.active or fx.active or pcx.active or pcx.cleanup.active or pcx.transition.active or rtx.active or rtx.cleanup.active or ed.active or ed.transition.active)..",\"caseIndex\":"..case_for_state..",\"functionReturnSeen\":"..bool(function_return_seen)..",\"orderWriteSeen\":"..bool(order_write_seen or tx.orderWriteSeen)..",\"pendingRngCall\":"..pending_rng_json()..",\"rolesAtPc\":"..roles_json(current_pc)..",\"transaction\":"..transaction_state_json()..",\"fulfillment\":"..fulfillment_state_json()..",\"precommit\":"..precommit_state_json()..",\"promptRouting\":"..routing_state_json()..",\"equipDecision\":"..equip_decision_state_json().."}"
end
...
edi.sessionSpanRoles={"member-list","held-items","equipment-type","equippability","recipient-cancel-terminal-boundary-shim","full-inventory-terminal-boundary-shim","non-equippable-terminal-boundary-shim","full-inventory-abort-presentation-skip","terminal-done-joint-redirect","equip-decision-prompt","equip-decision-prompt-presentation-skip","current-cursed-terminal-boundary-shim","noncursed-terminal-boundary-shim"}
local function validate_instrumented_rom()
  local ir=edi.instrumentedRom;expect(type(ir)=="table" and ir.spanCount==13 and type(ir.spans)=="table" and #ir.spans==ir.spanCount,"instrumented ROM runtime span count drift")
  local ranges={}
  for index,role in ipairs(edi.sessionSpanRoles) do
    ...
    local original,patched=hex_bytes(span.originalHex),hex_bytes(span.patchedHex);expect(#original==span.widthBytes and #patched==span.widthBytes and (span.widthBytes==6 or ((role=="equip-decision-prompt-presentation-skip" or role=="full-inventory-abort-presentation-skip") and span.widthBytes==4) or (role=="terminal-done-joint-redirect" and span.widthBytes==10)),"instrumented ROM runtime span width drift: "..role)
  ...
  expect(edi.instrumentedRomReadbacks["equip-decision-prompt-presentation-skip"] and edi.instrumentedRomReadbacks["full-inventory-abort-presentation-skip"] and edi.instrumentedRomReadbacks["terminal-done-joint-redirect"],"instrumented ROM prompt/Done readback missing");edi.promptPresentationSkipReadback=true;rti.abortPresentationSkipReadback=true;rti.doneRedirectReadback=true
end
...
local function precommit_terminal_shim(role,address)
  for _,shim in ipairs(p.terminalShims) do
    if shim.role==role then
      local expected_type=role=="recipient-cancel-terminal-boundary-shim" and "terminal-jmp" or "prompt-jsr";local expected_hex=role=="recipient-cancel-terminal-boundary-shim" and "4EF900FF6D20" or "4EB900FF6D20"
      expect(shim.type==expected_type and shim.boundaryAddress==address,"precommit terminal shim source identity drift: "..role)
      expect(shim.generatedStubTarget==precommit_state.terminalStub and shim.patchedHex==expected_hex,"precommit terminal shim generated target/opcode drift: "..role)
      expect(shim.returnAddress==(expected_type=="prompt-jsr" and address+6 or nil),"precommit terminal shim return ABI drift: "..role)
      return shim
    end
  end
...
local function validate_precommit_terminal_boundary(shim)
  local original=hex_bytes(shim.originalHex);local patched=hex_bytes(shim.patchedHex);local opcode=shim.type=="terminal-jmp" and 0xF9 or 0xB9;expect(#original==6 and #patched==6 and patched[1]==0x4E and patched[2]==opcode,"precommit terminal/prompt shim opcode drift")
  expect(shim.returnAddress==(shim.type=="prompt-jsr" and shim.boundaryAddress+6 or nil),"precommit terminal/prompt shim return relation drift")
...
local function fail_callback(message)
  ...
  local case=nil
  if mode=="helper" or current_role=="case-entry" then case=current_helper() elseif mode=="transaction" or current_role=="transaction-case-entry" then case=current_transaction() elseif mode=="fulfillment" or current_role=="fulfillment-case-entry" then case=current_fulfillment() elseif mode=="precommit" or mode=="precommit-cleanup" or current_role=="precommit-case-entry" or pcx.transition.active then case=current_precommit() elseif mode=="prompt-routing" or mode=="prompt-routing-cleanup" or current_role=="prompt-routing-case-entry" then case=current_routing() elseif mode=="equip-decision" or current_role=="equip-decision-case-entry" or ed.transition.active then case=current_equip() end
  ...
  local stack={}
  if current_role=="prompt-routing-dispatch-stub" or current_role=="prompt-routing-done-movem-stub-entry" or current_role=="prompt-routing-done-rts-stub-entry" then
    stack=rtx.dispatchStackDiagnostic or {}
  elseif current_role=="precommit-cleanup-equippability-effective-return" then
    stack=pcx.cleanupStackDiagnostic or {}
  end
...
function routing_members_json(values)
  local parts={};for _,value in ipairs(values) do parts[#parts+1]=value==-1 and "null" or tostring(value) end;return "["..table.concat(parts,",").."]"
end
function routing_record_json(record)
  return "{\"id\":"..json_string(record.id)..",\"promptFamily\":"..json_string(record.promptFamily)..",\"promptResult\":"..record.promptResult..",\"itemIndex\":"..record.itemIndex..",\"attemptCount\":"..record.attemptCount..",\"selectedMembers\":"..routing_members_json(record.selectedMembers)..",\"ordersBefore\":"..array_json(record.ordersBefore)..",\"ordersAfter\":"..array_json(record.ordersAfter)..",\"fulfilledOrdersBefore\":"..record.fulfilledOrdersBefore..",\"fulfilledOrdersAfter\":"..record.fulfilledOrdersAfter..",\"routeTerminal\":"..json_string(record.routeTerminal)..",\"routeTerminalPc\":"..record.routeTerminalPc..",\"completionTerminal\":"..json_string(record.completionTerminal)..",\"completionTerminalPc\":"..record.completionTerminalPc..",\"addItemMutationObserved\":false,\"orderMutationObserved\":false,\"fulfilledOrdersMutationObserved\":false,\"callbackChronology\":"..chronology_json(record.callbackChronology).."}"
end
...
function write_output()
  local helpers,transactions,fulfillments,precommits,routings,equips,helper_order,transaction_order,fulfillment_order,precommit_order,routing_order,equip_order={}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}
  ...
  for _,record in ipairs(routing_records) do routings[#routings+1]=routing_record_json(record) end
  ...
  for _,id in ipairs(config.promptRoutingCaseOrder) do routing_order[#routing_order+1]=json_string(id) end
  ...
  expect(#routing_records==#config.promptRoutingCases and rti.promptBoundaryReadbacks["full-inventory-terminal-boundary-shim"] and rti.promptBoundaryReadbacks["non-equippable-terminal-boundary-shim"] and rti.abortPresentationSkipReadback and rti.generatedPromptStubWriteReadback and rti.doneRedirectReadback and rti.jointDispatcherWriteReadback and rti.generatedDoneMovemStubWriteReadback and rti.generatedDoneRtsStubWriteReadback and rti.doneMovemStackReadback and rti.doneRtsStackReadback,"prompt-routing instrumentation/readback state drift")
  local equip_terminal_roles={};for _,shim in ipairs(e.terminalShims) do expect(edi.terminalBoundarySitesReadback[shim.role],"equip-decision terminal instrumentation readback missing: "..shim.role);equip_terminal_roles[#equip_terminal_roles+1]=json_string(shim.role) end;expect(edi.terminalBoundarySitesReadback["do-not-equip-terminal-boundary-shim"],"equip-decision joint terminal instrumentation readback missing");equip_terminal_roles[#equip_terminal_roles+1]=json_string("do-not-equip-terminal-boundary-shim")
  ...
  local output=assert(io.open(config.outputPath,"w"));output:write("{\"system\":"..json_string(emu.getsystemid())..",\"core\":"..json_string(config.core)..",\"id\":"..json_string(config.id)..",\"caseOrder\":["..table.concat(helper_order,",").."],\"records\":["..table.concat(helpers,",").."],\"transactionCaseOrder\":["..table.concat(transaction_order,",").."],\"transactionRecords\":["..table.concat(transactions,",").."],\"fulfillmentCaseOrder\":["..table.concat(fulfillment_order,",").."],\"fulfillmentRecords\":["..table.concat(fulfillments,",").."],\"precommitCaseOrder\":["..table.concat(precommit_order,",").."],\"precommitRecords\":["..table.concat(precommits,",").."],\"promptRoutingCaseOrder\":["..table.concat(routing_order,",").."],\"promptRoutingRecords\":["..table.concat(routings,",").."],\"equipDecisionCaseOrder\":["..table.concat(equip_order,",").."],\"equipDecisionRecords\":["..table.concat(equips,",").."],\"callbacksCleared\":0,\"precommitInstrumentation\":{\"serviceCallSitesReadback\":"..service_roles..",\"terminalBoundarySitesReadback\":"..terminal_roles..",\"generatedServiceStubWritesReadback\":true,\"generatedResultStubWritesReadback\":true},\"precommitRestoration\":{\"dialogueNameIndex1WordRestored\":true,\"selectedItemIndexWordRestored\":true,\"currentItemSubmenuActionByteRestored\":true},\"promptRoutingInstrumentation\":{\"promptBoundarySitesReadback\":[\"full-inventory-terminal-boundary-shim\",\"non-equippable-terminal-boundary-shim\"],\"fullInventoryAbortPresentationSkipReadback\":true,\"generatedPromptStubWriteReadback\":true,\"doneRedirectReadback\":true,\"jointDispatcherWriteReadback\":true,\"generatedDoneMovemStubWriteReadback\":true,\"generatedDoneRtsStubWriteReadback\":true,\"doneMovemStackReadback\":true,\"doneRtsStackReadback\":true},\"equipDecisionInstrumentation\":{\"promptCallSiteReadback\":true,\"promptPresentationSkipReadback\":true,\"generatedPromptStubWriteReadback\":true,\"terminalBoundarySitesReadback\":["..table.concat(equip_terminal_roles,",").."],\"generatedTerminalStubWriteReadback\":true},\"restoration\":{\"currentGoldLongRestored\":true,\"randomSeedWordRestored\":true,\"orderWordsRestored\":true,\"flag80OwningByteRestored\":true,\"clientCombatantRecordsRestored\":true}}");output:close()
end
...
function ed_joint_terminal_redirect(joint)
  if mode~="equip-decision" then return end
  local case=assert(current_equip(),"equip-decision joint terminal case exhausted");expect(ed.active and ed.expectedTerminal=="do-not-equip-pre-presentation" and joint.role=="terminal-done-joint-redirect" and joint.address==e.doNotEquipBoundaryAddress and joint.doneOffsetBytes==4 and joint.dispatcher.address==0xFF6D90,"equip-decision joint terminal source identity drift");expect(equal_arrays(read_bytes(joint.address,#hex_bytes(joint.patchedHex)),hex_bytes(joint.patchedHex)),"equip-decision joint terminal session readback drift");edi.terminalBoundarySitesReadback["do-not-equip-terminal-boundary-shim"]=true;ed_event("equip-decision-do-not-equip-pre-presentation",joint.address,nil,joint.dispatcher.address,nil);ed.terminal="do-not-equip-pre-presentation";ed.terminalPc=joint.address;ed.jointDispatcherPending=true
end
...
function dispatch_joint_terminal_done_dispatcher(address)
  local done=p.promptRouting.doneNeutralBoundary;local joint=done.jointRedirect
  if address~=joint.dispatcher.address then return false end
  if mode=="prompt-routing" then rtx_joint_terminal_done_dispatcher(done);return true end
  if mode=="equip-decision" then ed_joint_terminal_done_dispatcher(joint);return true end
  error("joint terminal/Done dispatcher outside deterministic mode")
end
...
function ed_joint_terminal_done_dispatcher(joint)
  local target=0xFF6D60;set_expectation("equip-decision","equip-decision-joint-terminal-done-dispatcher",joint.dispatcher.address,nil,target,nil);expect(ed.active and emu.getregister("M68K PC")==joint.dispatcher.address and ed.jointDispatcherPending and ed.expectedTerminal=="do-not-equip-pre-presentation" and joint.dispatcher.address==0xFF6D90 and joint.dispatcher.jmpWidthBytes==6,"equip-decision joint dispatcher pending/identity drift");write_bytes(joint.dispatcher.address,{0x4E,0xF9,target>>24&0xFF,target>>16&0xFF,target>>8&0xFF,target&0xFF});expect(equal_arrays(read_bytes(joint.dispatcher.address,6),{0x4E,0xF9,target>>24&0xFF,target>>16&0xFF,target>>8&0xFF,target&0xFF}),"equip-decision joint dispatcher write/readback drift");ed.jointDispatcherPending=false
end
...
function pcx.cleanup_event(role,address,call_pc,target_pc,return_pc)
  if mode~="precommit-cleanup" and mode~="prompt-routing-cleanup" then return end
  set_expectation(mode,role,address,call_pc,target_pc,return_pc);expect(pcx.cleanup.active,"precommit cleanup callback outside active cleanup");expect(emu.getregister("M68K PC")==address,"precommit cleanup callback PC drift: "..role)
end
...
function pcx.cleanup_add_call() if mode=="precommit-cleanup" or mode=="prompt-routing-cleanup" then pcx.cleanup_event("precommit-cleanup-add-item-call",u.addItemCallAddress,u.addItemCallAddress,u.addItemInstructionTargetAddress,u.addItemReturnAddress) end end
function pcx.cleanup_add_instruction() if mode=="precommit-cleanup" or mode=="prompt-routing-cleanup" then pcx.cleanup_event("precommit-cleanup-add-item-instruction-target",u.addItemInstructionTargetAddress,u.addItemCallAddress,u.addItemInstructionTargetAddress,u.addItemEffectiveTargetAddress) end end
function pcx.cleanup_add_target() if mode=="precommit-cleanup" or mode=="prompt-routing-cleanup" then pcx.cleanup_event("precommit-cleanup-add-item-effective-target",u.addItemEffectiveTargetAddress,u.addItemCallAddress,u.addItemEffectiveTargetAddress,u.addItemEffectiveReturnAddress) end end
...
function pcx.cleanup_equippability_call() if mode=="precommit-cleanup" or mode=="prompt-routing-cleanup" then local q=p.cleanupEquippability;pcx.cleanup_event("precommit-cleanup-equippability-call",q.callAddress,q.callAddress,q.instructionTargetAddress,q.returnAddress);expect(pcx.cleanup.fulfilledOrdersIncrementSeen,"precommit cleanup equippability before order increment") end end
function pcx.cleanup_equippability_instruction() if mode=="precommit-cleanup" or mode=="prompt-routing-cleanup" then local q=p.cleanupEquippability;pcx.cleanup_event("precommit-cleanup-equippability-instruction-target",q.instructionTargetAddress,q.callAddress,q.instructionTargetAddress,q.effectiveTargetAddress) end end
function pcx.cleanup_equippability_target() if mode=="precommit-cleanup" or mode=="prompt-routing-cleanup" then local q=p.cleanupEquippability;pcx.cleanup_event("precommit-cleanup-equippability-effective-target",q.effectiveTargetAddress,q.callAddress,q.effectiveTargetAddress,q.effectiveReturnAddress) end end
function pcx.cleanup_equippability_return()
  if mode~="precommit-cleanup" and mode~="prompt-routing-cleanup" then return end
  local q=p.cleanupEquippability;pcx.cleanup_event("precommit-cleanup-equippability-effective-return",q.effectiveReturnAddress,q.callAddress,q.effectiveTargetAddress,q.returnAddress);local cleanup=pcx.cleanup.case;local stack=emu.getregister("M68K A7")&0xFFFFFF;local routing_cleanup=mode=="prompt-routing-cleanup";local expected_top=stack_top-(routing_cleanup and config.promptRoutingCleanupStackDepthBytes or config.precommitCleanupStackDepthBytes);local actual_return=memory.read_u32_be(stack,"M68K BUS");pcx.cleanupStackDiagnostic={expectedTop=expected_top,actualTop=stack,expectedReturn=q.returnAddress,actualReturn=actual_return};expect(stack==expected_top and actual_return==q.returnAddress,"precommit cleanup safe-return stack relation drift");local carry=(emu.getregister("M68K SR")&1)~=0;expect(carry==cleanup.equippableCarrySet,"precommit cleanup equippability carry drift");local target=routing_cleanup and routing_pc(routing_index)+20 or precommit_pc(precommit_index)+20;memory.write_u32_be(stack,target,"M68K BUS");expect(memory.read_u32_be(stack,"M68K BUS")==target,"precommit cleanup safe-return rewrite drift");pcx.cleanup.equippabilityCarrySet=carry;pcx.cleanup.active=false;mode=routing_cleanup and "prompt-routing" or "precommit"
end
...
function rtx_event(role,address,call_pc,target_pc,return_pc)
  if mode~="prompt-routing" then return end
  set_expectation("prompt-routing",role,address,call_pc,target_pc,return_pc);expect(rtx.active and emu.getregister("M68K PC")==address,"prompt-routing callback PC/state drift: "..role)
  local kept={
    ["prompt-routing-selection-loop-entry"]=true,["prompt-routing-capacity-branch"]=true,["prompt-routing-equippability-branch"]=true,["prompt-routing-full-inventory-boundary-jsr"]=true,["prompt-routing-non-equippable-boundary-jsr"]=true,["prompt-routing-dispatch-stub"]=true,["prompt-routing-controlled-result-stub-entry"]=true,["prompt-routing-controlled-result-stub-jmp-boundary"]=true,["prompt-routing-full-inventory-compare"]=true,["prompt-routing-full-inventory-branch"]=true,["prompt-routing-non-equippable-compare"]=true,["prompt-routing-non-equippable-branch"]=true,["prompt-routing-recipient-cancel-terminal"]=true,["prompt-routing-full-inventory-abort-presentation-skip"]=true,["prompt-routing-done-redirect"]=true,["prompt-routing-joint-terminal-done-dispatcher"]=true,["prompt-routing-done-movem-stub-entry"]=true,["prompt-routing-done-rts-stub-entry"]=true,["prompt-routing-add-item-boundary"]=true,["prompt-routing-case-result"]=true}
  if kept[role] then rtx.chronology[#rtx.chronology+1]={role=role,pc=address} end
end
routing_role_prefix="prompt-routing-"
function rtx_attempt() local case=assert(current_routing(),"prompt-routing case table exhausted");return assert(case.attempts[rtx.attemptIndex],"prompt-routing selection attempt exhausted") end
function rtx_service_call(role,spec)
  if mode~="prompt-routing" then return end
  local shim=precommit_shim(role,spec);rtx_event(routing_role_prefix..role.."-controlled-service-call-shim",spec.callAddress,spec.callAddress,shim.generatedStubTarget,spec.returnAddress);expect(rtx.pendingService==nil,"prompt-routing overlapping service call");local attempt=rtx_attempt();if role=="held-items" then expect(attempt.heldItemsCountResult~=nil,"prompt-routing held-items result missing") elseif role=="equipment-type" then expect(attempt.equipmentTypeResult~=nil,"prompt-routing equipment-type result missing") elseif role=="equippability" then expect(attempt.equippableCarrySetResult~=nil,"prompt-routing equippability result missing") end;validate_precommit_service_call(shim);write_precommit_service_stub(role,attempt);rtx.pendingService={role=role,callPc=spec.callAddress,targetPc=shim.generatedStubTarget,returnPc=spec.returnAddress};if role=="member-list" then rtx.memberListCallCount=rtx.memberListCallCount+1 elseif role=="held-items" then rtx.heldItemsCallCount=rtx.heldItemsCallCount+1 elseif role=="equipment-type" then rtx.equipmentTypeCallCount=rtx.equipmentTypeCallCount+1 else rtx.equippabilityCallCount=rtx.equippabilityCallCount+1 end
end
function rtx_service_stub()
  if mode~="prompt-routing" then return end
  local pending=assert(rtx.pendingService,"prompt-routing generated service stub without source call");rtx_event("prompt-routing-generated-service-stub",precommit_state.serviceStub,pending.callPc,pending.targetPc,pending.returnPc)
end
function rtx_service_return(role,spec)
  if mode~="prompt-routing" then return end
  local pending=assert(rtx.pendingService,"prompt-routing service return without pending call");expect(pending.role==role and pending.callPc==spec.callAddress and pending.returnPc==spec.returnAddress,"prompt-routing pending service relation drift: "..role);rtx_event(routing_role_prefix..role.."-original-return",spec.returnAddress,spec.callAddress,pending.targetPc,spec.returnAddress);rtx.pendingService=nil
end
function rtx_route_record(route,route_pc,completion,completion_pc)
  local case=current_routing();local orders=read_orders();local fulfilled=memory.read_u16_be(fulfillment_frame_address()+p.frameOffsetsBytes.fulfilledOrdersNumber,"M68K BUS");expect(equal_arrays(orders,case.ordersBefore) and fulfilled==case.fulfilledOrdersBefore,"prompt-routing mutation before terminal");rtx.routeTerminal=route;rtx.completionTerminal=completion;rtx.record={id=case.id,promptFamily=case.promptFamily,promptResult=case.promptResult,itemIndex=case.itemIndex,attemptCount=rtx.attemptIndex,selectedMembers=rtx.selectedMembers,ordersBefore=case.ordersBefore,ordersAfter=orders,fulfilledOrdersBefore=case.fulfilledOrdersBefore,fulfilledOrdersAfter=fulfilled,routeTerminal=route,routeTerminalPc=route_pc,completionTerminal=completion,completionTerminalPc=completion_pc,addItemMutationObserved=false,orderMutationObserved=false,fulfilledOrdersMutationObserved=false,callbackChronology=rtx.chronology}
end
function rtx_selection_loop()
  if mode~="prompt-routing" then return end
  rtx_event("prompt-routing-selection-loop-entry",p.selectionLoopAddress,nil,nil,nil)
end
function rtx_member_call() rtx_service_call("member-list",p.memberList) end
function rtx_member_return() rtx_service_return("member-list",p.memberList) end
function rtx_member_compare()
  if mode~="prompt-routing" then return end
  rtx_event("prompt-routing-member-cancel-compare",p.memberCancelCompareAddress,p.memberList.callAddress,p.memberList.instructionTargetAddress,p.memberList.returnAddress);local attempt=rtx_attempt();expect(word(emu.getregister("M68K D0"))==word(attempt.selectedMemberResult),"prompt-routing member result drift");rtx.selectedMembers[#rtx.selectedMembers+1]=attempt.selectedMemberResult
end
function rtx_member_branch() if mode=="prompt-routing" then rtx_event("prompt-routing-member-cancel-branch",p.memberCancelBranchAddress,nil,nil,nil) end end
function rtx_held_call() rtx_service_call("held-items",p.heldItems) end
function rtx_held_return() rtx_service_return("held-items",p.heldItems) end
function rtx_capacity_compare() if mode=="prompt-routing" then rtx_event("prompt-routing-capacity-compare",p.capacityCompareAddress,p.heldItems.callAddress,p.heldItems.instructionTargetAddress,p.heldItems.returnAddress);expect(word(emu.getregister("M68K D2"))==rtx_attempt().heldItemsCountResult,"prompt-routing held-items result drift") end end
function rtx_capacity_branch() if mode=="prompt-routing" then rtx_event("prompt-routing-capacity-branch",p.capacityBranchAddress,nil,nil,nil) end end
function rtx_type_call() rtx_service_call("equipment-type",p.equipmentType) end
function rtx_type_return() rtx_service_return("equipment-type",p.equipmentType) end
function rtx_type_compare() if mode=="prompt-routing" then rtx_event("prompt-routing-equipment-type-compare",p.equipmentTypeCompareAddress,p.equipmentType.callAddress,p.equipmentType.instructionTargetAddress,p.equipmentType.returnAddress);expect(word(emu.getregister("M68K D2"))==rtx_attempt().equipmentTypeResult,"prompt-routing equipment type drift") end end
function rtx_tool_branch() if mode=="prompt-routing" then rtx_event("prompt-routing-tool-admission-branch",p.toolAdmissionBranchAddress,nil,nil,nil) end end
function rtx_equippability_call() rtx_service_call("equippability",p.equippability) end
function rtx_equippability_return() rtx_service_return("equippability",p.equippability) end
function rtx_equippability_branch() if mode=="prompt-routing" then rtx_event("prompt-routing-equippability-branch",p.equippabilityBranchAddress,p.equippability.callAddress,p.equippability.instructionTargetAddress,p.equippability.returnAddress);expect(((emu.getregister("M68K SR")&1)~=0)==rtx_attempt().equippableCarrySetResult,"prompt-routing equippability carry drift") end end
function rtx_prompt_boundary(family,role,address)
  if mode~="prompt-routing" then return end
  local shim=precommit_terminal_shim(role,address);validate_precommit_terminal_boundary(shim);local source=p.promptRouting[family=="full-inventory" and "fullInventory" or "nonEquippable"];local stub=p.promptRouting.controlledResultStub;rtx_event(routing_role_prefix..family.."-boundary-jsr",address,address,precommit_state.terminalStub,shim.returnAddress);expect(rtx.pendingService==nil and rtx.controlledPrompt==nil,"prompt-routing prompt boundary pending state drift");local value=word(current_routing().promptResult);expect(stub.address==0xFF6D80 and stub.moveWidthBytes==4 and stub.jmpAddress==stub.address+stub.moveWidthBytes and stub.jmpWidthBytes==6 and stub.sizeBytes==stub.moveWidthBytes+stub.jmpWidthBytes,"prompt-routing controlled-result stub boundary config drift");rtx.terminalStubPurpose="prompt";rtx.dispatchStackDiagnostic=nil;rtx.promptBoundaryReturnAddress=shim.returnAddress;rtx.controlledPrompt={family=family,resultWord=value,compareAddress=source.compareAddress};write_bytes(precommit_state.terminalStub,{0x4E,0xF9,stub.address>>24&0xFF,stub.address>>16&0xFF,stub.address>>8&0xFF,stub.address&0xFF});expect(equal_arrays(read_bytes(precommit_state.terminalStub,6),{0x4E,0xF9,stub.address>>24&0xFF,stub.address>>16&0xFF,stub.address>>8&0xFF,stub.address&0xFF}),"prompt-routing dispatch stub write drift");write_bytes(stub.address,{0x30,0x3C,value>>8,value&0xFF,0x4E,0xF9,source.compareAddress>>24&0xFF,source.compareAddress>>16&0xFF,source.compareAddress>>8&0xFF,source.compareAddress&0xFF});expect(equal_arrays(read_bytes(stub.address,stub.sizeBytes),{0x30,0x3C,value>>8,value&0xFF,0x4E,0xF9,source.compareAddress>>24&0xFF,source.compareAddress>>16&0xFF,source.compareAddress>>8&0xFF,source.compareAddress&0xFF}),"prompt-routing controlled-result stub write drift");rti.promptBoundaryReadbacks[role]=true;rti.generatedPromptStubWriteReadback=true;rtx.family=family
end
function rtx_dispatch_stub()
  if mode~="prompt-routing" then return end
  if rtx.terminalStubPurpose=="prompt" then
    local expected_return=rtx.promptBoundaryReturnAddress;local stub=p.promptRouting.controlledResultStub;rtx_event("prompt-routing-dispatch-stub",precommit_state.terminalStub,nil,stub.address,expected_return);local stack_contract=p.promptRouting.dispatchStack;local stack=emu.getregister("M68K A7")&0xFFFFFF;local actual_return=memory.read_u32_be(stack,"M68K BUS");local expected_top=stack_top-stack_contract.totalBytes;rtx.dispatchStackDiagnostic={expectedTop=expected_top,actualTop=stack,expectedReturn=expected_return,actualReturn=actual_return}
    expect(memory.read_u16_be(precommit_state.terminalStub,"M68K BUS")==0x4EF9 and memory.read_u32_be(precommit_state.terminalStub+2,"M68K BUS")==stub.address,"prompt-routing dispatch stub opcode/target drift")
    expect(expected_return~=nil,"prompt-routing dispatch prompt-return identity drift")
    expect(stack_contract.entryJsrReturnBytes==4 and stack_contract.promptBoundaryJsrReturnBytes==4 and stack_contract.totalBytes==stack_contract.entryJsrReturnBytes+stack_contract.promptBoundaryJsrReturnBytes,"prompt-routing dispatch stack-depth contract drift")
    expect(stack==expected_top,"prompt-routing dispatch A7 depth drift")
    expect(actual_return==expected_return,"prompt-routing dispatch stack return drift")
  elseif rtx.terminalStubPurpose=="recipient-terminal" then
    local target=routing_pc(routing_index)+20;rtx_event("prompt-routing-dispatch-stub",precommit_state.terminalStub,nil,target,nil);expect(memory.read_u16_be(precommit_state.terminalStub,"M68K BUS")==0x4EF9 and memory.read_u32_be(precommit_state.terminalStub+2,"M68K BUS")==target,"prompt-routing recipient result stub opcode/target drift")
  else error("prompt-routing terminal stub purpose drift") end
  rtx.terminalStubPurpose=nil
end
function rtx_prompt_stub_entry()
  if mode~="prompt-routing" then return end
  local stub=p.promptRouting.controlledResultStub;local pending=rtx.controlledPrompt;expect(pending~=nil and pending.family==rtx.family and pending.compareAddress==p.promptRouting[rtx.family=="full-inventory" and "fullInventory" or "nonEquippable"].compareAddress,"prompt-routing controlled-result stub pending identity drift");rtx_event("prompt-routing-controlled-result-stub-entry",stub.address,nil,stub.jmpAddress,nil);expect(equal_arrays(read_bytes(stub.address,stub.sizeBytes),{0x30,0x3C,pending.resultWord>>8,pending.resultWord&0xFF,0x4E,0xF9,pending.compareAddress>>24&0xFF,pending.compareAddress>>16&0xFF,pending.compareAddress>>8&0xFF,pending.compareAddress&0xFF}),"prompt-routing controlled-result stub entry bytes drift")
end
function rtx_prompt_stub_jmp()
  if mode~="prompt-routing" then return end
  local stub=p.promptRouting.controlledResultStub;local pending=rtx.controlledPrompt;expect(pending~=nil and pending.family==rtx.family,"prompt-routing controlled-result JMP pending identity drift");rtx_event("prompt-routing-controlled-result-stub-jmp-boundary",stub.jmpAddress,nil,pending.compareAddress,nil);expect(memory.read_u16_be(stub.jmpAddress,"M68K BUS")==0x4EF9 and memory.read_u32_be(stub.jmpAddress+2,"M68K BUS")==pending.compareAddress,"prompt-routing controlled-result JMP bytes drift");expect(word(emu.getregister("M68K D0"))==pending.resultWord,"prompt-routing controlled prompt result drift");rtx.controlledPrompt=nil
end
function rtx_compare(family) if mode=="prompt-routing" then local source=p.promptRouting[family=="full-inventory" and "fullInventory" or "nonEquippable"];rtx_event(routing_role_prefix..family.."-compare",source.compareAddress,nil,nil,nil);expect(rtx.controlledPrompt==nil and word(emu.getregister("M68K D0"))==word(current_routing().promptResult),"prompt-routing compare result drift") end end
function rtx_branch(family)
  if mode~="prompt-routing" then return end
  local source=p.promptRouting[family=="full-inventory" and "fullInventory" or "nonEquippable"];rtx_event(routing_role_prefix..family.."-branch",source.retryBranchAddress,nil,source.retryTargetAddress,nil);local retry=(family=="full-inventory" and current_routing().promptResult==0) or (family=="non-equippable" and current_routing().promptResult~=0);if retry then rtx.attemptIndex=rtx.attemptIndex+1;expect(current_routing().attempts[rtx.attemptIndex]~=nil,"prompt-routing retry attempt missing") end
end
function rtx_recipient_terminal(address)
  if mode~="prompt-routing" then return end
  local shim=precommit_terminal_shim("recipient-cancel-terminal-boundary-shim",address);validate_precommit_terminal_boundary(shim);rtx_event("prompt-routing-recipient-cancel-terminal",address,nil,precommit_state.terminalStub,nil);rtx.terminalStubPurpose="recipient-terminal";rtx.dispatchStackDiagnostic=nil;write_precommit_result_stub(routing_pc(routing_index)+20);rtx_route_record("selection-loop",p.selectionLoopAddress,"recipient-cancel-pre-presentation",address)
end
function rtx_abort_skip()
  if mode~="prompt-routing" then return end
  local skip=p.fullInventoryAbortPresentationSkip;rtx_event("prompt-routing-full-inventory-abort-presentation-skip",skip.boundaryAddress,nil,skip.targetAddress,nil);expect(current_routing().promptFamily=="full-inventory" and current_routing().promptResult~=0,"prompt-routing abort skip polarity drift");expect(equal_arrays(read_bytes(skip.boundaryAddress,4),hex_bytes(skip.patchedHex)),"prompt-routing abort skip readback drift")
end
function rtx_done_redirect()
  if mode~="prompt-routing" then return end
  local done=p.promptRouting.doneNeutralBoundary;local joint=done.jointRedirect;rtx_event("prompt-routing-done-redirect",joint.address,nil,joint.dispatcher.address,nil);expect(not rtx.jointDispatcherPending and not rtx.doneMovemPending and not rtx.doneRtsPending and done.restoreMask==0x03FF and done.restoreByteCount==40 and done.movemWidthBytes==4 and done.sourceRtsAddress==done.address+done.movemWidthBytes and done.rtsWidthBytes==2 and joint.role=="terminal-done-joint-redirect" and joint.type=="joint-terminal-done-jmp" and joint.address==done.address-joint.doneOffsetBytes and joint.doneOffsetBytes==4 and joint.originalHex=="4E4500D14CDF03FF4E75" and joint.unreachableFillHex=="4AFC4AFC" and joint.dispatcher.address==0xFF6D90 and joint.dispatcher.jmpWidthBytes==6 and equal_arrays(read_bytes(joint.address,10),hex_bytes(joint.patchedHex)),"prompt-routing joint Done redirect/source-equivalent contract drift");rti.doneRedirectReadback=true;rtx.jointDispatcherPending=true;rtx_route_record("done",done.address,"done",done.address)
end
function rtx_joint_terminal_done_dispatcher(joint)
  local target=joint.movemStub.address;rtx_event("prompt-routing-joint-terminal-done-dispatcher",joint.jointRedirect.dispatcher.address,nil,target,nil);expect(rtx.jointDispatcherPending and not rtx.doneMovemPending and not rtx.doneRtsPending and joint.jointRedirect.dispatcher.jmpWidthBytes==6 and target==0xFF6DA0,"prompt-routing joint dispatcher pending/target drift");write_bytes(joint.jointRedirect.dispatcher.address,{0x4E,0xF9,target>>24&0xFF,target>>16&0xFF,target>>8&0xFF,target&0xFF});expect(equal_arrays(read_bytes(joint.jointRedirect.dispatcher.address,6),{0x4E,0xF9,target>>24&0xFF,target>>16&0xFF,target>>8&0xFF,target&0xFF}),"prompt-routing joint dispatcher write/readback drift");rti.jointDispatcherWriteReadback=true;rtx.jointDispatcherPending=false;rtx.doneMovemPending=true
end
function rtx_done_movem_stub()
  if mode~="prompt-routing" then return end
  local done=p.promptRouting.doneNeutralBoundary;local stub=done.movemStub;local stack_contract=p.promptRouting.dispatchStack;local stack=emu.getregister("M68K A7")&0xFFFFFF;local expected_top=stack_top-stack_contract.totalBytes;local expected_return=routing_pc(routing_index)+20;local actual_return=memory.read_u32_be(stack+done.restoreByteCount,"M68K BUS");rtx.dispatchStackDiagnostic={expectedTop=expected_top,actualTop=stack,expectedReturn=expected_return,actualReturn=actual_return};rtx_event("prompt-routing-done-movem-stub-entry",stub.address,nil,stub.rtsStubAddress,nil);expect(rtx.doneMovemPending and not rtx.doneRtsPending and stub.movemHex=="4CDF03FF" and stub.movemWidthBytes==4 and stub.jmpAddress==stub.address+stub.movemWidthBytes and stub.jmpWidthBytes==6 and stub.rtsStubAddress==done.rtsStub.address and stub.sizeBytes==stub.movemWidthBytes+stub.jmpWidthBytes,"prompt-routing Done MOVEM stub identity drift");expect(equal_arrays(read_bytes(stub.address,stub.sizeBytes),{0x4C,0xDF,0x03,0xFF,0x4E,0xF9,stub.rtsStubAddress>>24&0xFF,stub.rtsStubAddress>>16&0xFF,stub.rtsStubAddress>>8&0xFF,stub.rtsStubAddress&0xFF}),"prompt-routing Done MOVEM stub bytes drift");expect(stack==expected_top,"prompt-routing Done MOVEM pre-stack A7 drift");expect(actual_return==expected_return,"prompt-routing Done MOVEM synthetic return drift");rti.doneMovemStackReadback=true;rtx.doneMovemPending=false;rtx.doneRtsPending=true
end
function rtx_done_rts_stub()
  if mode~="prompt-routing" then return end
  local done=p.promptRouting.doneNeutralBoundary;local stub=done.rtsStub;local stack_contract=p.promptRouting.dispatchStack;local stack=emu.getregister("M68K A7")&0xFFFFFF;local expected_top=stack_top-stack_contract.totalBytes+done.restoreByteCount;local expected_return=routing_pc(routing_index)+20;local actual_return=memory.read_u32_be(stack,"M68K BUS");rtx.dispatchStackDiagnostic={expectedTop=expected_top,actualTop=stack,expectedReturn=expected_return,actualReturn=actual_return};rtx_event("prompt-routing-done-rts-stub-entry",stub.address,nil,expected_return,nil);expect(not rtx.doneMovemPending and rtx.doneRtsPending and stub.rtsHex=="4E75" and stub.sizeBytes==2,"prompt-routing Done RTS stub identity drift");expect(equal_arrays(read_bytes(stub.address,stub.sizeBytes),{0x4E,0x75}),"prompt-routing Done RTS stub bytes drift");expect(stack==expected_top,"prompt-routing Done RTS post-MOVEM A7 drift");expect(actual_return==expected_return,"prompt-routing Done RTS return drift");rti.doneRtsStackReadback=true;rtx.doneRtsPending=false
end
function write_done_stubs(done)
  local movem,rts=done.movemStub,done.rtsStub
  local movem_bytes={0x4C,0xDF,0x03,0xFF,0x4E,0xF9,rts.address>>24&0xFF,rts.address>>16&0xFF,rts.address>>8&0xFF,rts.address&0xFF}
  local rts_bytes={0x4E,0x75}
  expect(movem.address==0xFF6DA0 and movem.movemHex=="4CDF03FF" and movem.movemWidthBytes==4 and movem.jmpAddress==movem.address+movem.movemWidthBytes and movem.jmpWidthBytes==6 and movem.rtsStubAddress==rts.address and movem.sizeBytes==#movem_bytes and rts.address==0xFF6DB0 and rts.rtsHex=="4E75" and rts.sizeBytes==#rts_bytes,"prompt-routing Done generated stub config drift")
  write_bytes(movem.address,movem_bytes);write_bytes(rts.address,rts_bytes)
  expect(equal_arrays(read_bytes(movem.address,#movem_bytes),movem_bytes),"prompt-routing Done MOVEM stub write drift")
  expect(equal_arrays(read_bytes(rts.address,#rts_bytes),rts_bytes),"prompt-routing Done RTS stub write drift")
  rti.generatedDoneMovemStubWriteReadback=true;rti.generatedDoneRtsStubWriteReadback=true
end
function rtx_cleanup_case()
  local case=current_routing();local match=nil;local count=0;for _,candidate in ipairs(config.fulfillmentCases) do if candidate.id==case.cleanupFulfillmentCaseId then match=candidate;count=count+1 end end;expect(count==1,"prompt-routing cleanup fulfillment identity drift");local selected=c.orderSlotCount-match.ordersCounter;expect(match.itemIndex==case.itemIndex and selected>=0 and selected<c.orderSlotCount and case.ordersBefore[selected+1]==match.itemIndex and match.ordersBefore[selected+1]==match.itemIndex,"prompt-routing cleanup item/order identity drift");return match
end
function rtx_add_item()
  if mode~="prompt-routing" then return end
  rtx_event("prompt-routing-add-item-boundary",p.addItemEntryAddress,nil,nil,nil);local case=current_routing();local cleanup=rtx_cleanup_case();expect(current_routing().promptFamily=="non-equippable" and current_routing().promptResult==0 and cleanup.itemIndex==case.itemIndex,"prompt-routing AddItem acceptance identity drift");rtx_route_record("add-item",p.addItemEntryAddress,"add-item",p.addItemEntryAddress);write_record(cleanup.clientMember,original_records[cleanup.clientMember]);write_class(cleanup.clientMember,cleanup.recipientClass);write_item_words(cleanup.clientMember,cleanup.clientItemWordsBefore);write_orders(cleanup.ordersBefore);local frame=fulfillment_frame_address();memory.write_u16_be(frame+u.frameOffsetsBytes.clientClass,cleanup.recipientClass,"M68K BUS");memory.write_u16_be(frame+u.frameOffsetsBytes.clientMember,cleanup.clientMember,"M68K BUS");memory.write_u16_be(frame+u.frameOffsetsBytes.itemIndex,cleanup.itemIndex,"M68K BUS");memory.write_u16_be(frame+u.frameOffsetsBytes.ordersCounter,cleanup.ordersCounter,"M68K BUS");memory.write_u16_be(frame+u.frameOffsetsBytes.fulfilledOrdersNumber,cleanup.fulfilledOrdersBefore,"M68K BUS");pcx.cleanup={active=true,case=cleanup,addItemReturnSeen=false,orderReadSeen=false,orderClearedSeen=false,fulfilledOrdersIncrementSeen=false,equippabilityCarrySet=nil};mode="prompt-routing-cleanup"
end
function start_routing_case(index)
  expect(mode=="none" and index==routing_index,"prompt-routing case-entry dispatch drift");local case=assert(current_routing(),"prompt-routing case table exhausted");local entry=routing_pc(index);set_expectation("prompt-routing-case-entry","prompt-routing-case-entry",entry,entry+14,p.runtimeStartAddress,nil);expect(emu.getregister("M68K PC")==entry,"prompt-routing case-entry PC drift");write_orders(case.ordersBefore);local frame=fulfillment_frame_address();memory.write_u16_be(frame+p.frameOffsetsBytes.itemIndex,case.itemIndex,"M68K BUS");memory.write_u16_be(frame+p.frameOffsetsBytes.fulfilledOrdersNumber,case.fulfilledOrdersBefore,"M68K BUS");local done=p.promptRouting.doneNeutralBoundary;local stack_contract=p.promptRouting.dispatchStack;local done_stack=stack_top-stack_contract.totalBytes;expect(done.restoreMask==0x03FF and done.restoreByteCount==40 and done.sourceRtsAddress==done.address+done.movemWidthBytes and stack_contract.entryJsrReturnBytes==4 and stack_contract.promptBoundaryJsrReturnBytes==4 and stack_contract.totalBytes==stack_contract.entryJsrReturnBytes+stack_contract.promptBoundaryJsrReturnBytes,"prompt-routing Done restore-stack contract drift");write_done_stubs(done);for offset=0,done.restoreByteCount-4,4 do memory.write_u32_be(done_stack+offset,0,"M68K BUS") end;memory.write_u32_be(done_stack+done.restoreByteCount,entry+20,"M68K BUS");expect(memory.read_u32_be(done_stack+done.restoreByteCount,"M68K BUS")==entry+20,"prompt-routing Done synthetic return setup drift");rtx={active=true,attemptIndex=1,memberListCallCount=0,heldItemsCallCount=0,equipmentTypeCallCount=0,equippabilityCallCount=0,pendingService=nil,promptBoundaryReturnAddress=nil,terminalStubPurpose=nil,controlledPrompt=nil,doneMovemPending=false,doneRtsPending=false,dispatchStackDiagnostic=nil,selectedMembers={},routeTerminal="none",completionTerminal="none",frameCount=0,frameBudget=config.promptRoutingCaseFrameBudget,record=nil,chronology={},cleanup={active=false,case=nil,addItemReturnSeen=false,orderReadSeen=false,orderClearedSeen=false,fulfilledOrdersIncrementSeen=false,equippabilityCarrySet=nil}};mode="prompt-routing";if index==1 then status("milestone:prompt-routing-cases-entered") end
end
function finish_routing_case(index)
  if mode~="prompt-routing" then return end
  local result_pc=routing_pc(index)+20;rtx_event("prompt-routing-case-result",result_pc,nil,rtx.record and rtx.record.routeTerminalPc or nil,nil);expect(index==routing_index and rtx.record~=nil and rtx.routeTerminal~="none" and not rtx.doneMovemPending and not rtx.doneRtsPending,"prompt-routing result without route terminal");routing_records[#routing_records+1]=rtx.record;rtx.active=false;mode="none";routing_index=routing_index+1;if routing_index>#config.promptRoutingCases then equip_index=1;ed.transition={active=true,frameCount=0,frameBudget=180};status("milestone:equip-decision-transition-armed") end
end
...
local function finish_precommit_case(index)
  ...
  if precommit_index>#config.precommitCases then routing_index=1;status("milestone:prompt-routing-transition-armed") end
end
...
local function dispatch(address,entry)
  ...
  elseif entry.role=="equip-decision-do-not-equip-terminal-boundary-shim" then ed_joint_terminal_redirect(p.promptRouting.doneNeutralBoundary.jointRedirect)
  ...
  elseif entry.role=="prompt-routing-case-entry" then start_routing_case(entry.index)
  elseif entry.role=="prompt-routing-selection-loop-entry" then rtx_selection_loop()
  elseif entry.role=="prompt-routing-member-list-controlled-service-call-shim" then rtx_member_call()
  elseif entry.role=="prompt-routing-generated-service-stub" then rtx_service_stub()
  elseif entry.role=="prompt-routing-member-list-original-return" then rtx_member_return()
  elseif entry.role=="prompt-routing-member-cancel-compare" then rtx_member_compare()
  elseif entry.role=="prompt-routing-member-cancel-branch" then rtx_member_branch()
  elseif entry.role=="prompt-routing-held-items-controlled-service-call-shim" then rtx_held_call()
  elseif entry.role=="prompt-routing-held-items-original-return" then rtx_held_return()
  elseif entry.role=="prompt-routing-capacity-compare" then rtx_capacity_compare()
  elseif entry.role=="prompt-routing-capacity-branch" then rtx_capacity_branch()
  elseif entry.role=="prompt-routing-equipment-type-controlled-service-call-shim" then rtx_type_call()
  elseif entry.role=="prompt-routing-equipment-type-original-return" then rtx_type_return()
  elseif entry.role=="prompt-routing-equipment-type-compare" then rtx_type_compare()
  elseif entry.role=="prompt-routing-tool-admission-branch" then rtx_tool_branch()
  elseif entry.role=="prompt-routing-equippability-controlled-service-call-shim" then rtx_equippability_call()
  elseif entry.role=="prompt-routing-equippability-original-return" then rtx_equippability_return()
  elseif entry.role=="prompt-routing-equippability-branch" then rtx_equippability_branch()
  elseif entry.role=="prompt-routing-full-inventory-boundary-jsr" then rtx_prompt_boundary("full-inventory","full-inventory-terminal-boundary-shim",address)
  elseif entry.role=="prompt-routing-non-equippable-boundary-jsr" then rtx_prompt_boundary("non-equippable","non-equippable-terminal-boundary-shim",address)
  elseif entry.role=="prompt-routing-dispatch-stub" then rtx_dispatch_stub()
  elseif entry.role=="prompt-routing-controlled-result-stub-entry" then rtx_prompt_stub_entry()
  elseif entry.role=="prompt-routing-controlled-result-stub-jmp-boundary" then rtx_prompt_stub_jmp()
  elseif entry.role=="prompt-routing-full-inventory-compare" then rtx_compare("full-inventory")
  elseif entry.role=="prompt-routing-full-inventory-branch" then rtx_branch("full-inventory")
  elseif entry.role=="prompt-routing-non-equippable-compare" then rtx_compare("non-equippable")
  elseif entry.role=="prompt-routing-non-equippable-branch" then rtx_branch("non-equippable")
  elseif entry.role=="prompt-routing-recipient-cancel-terminal" then rtx_recipient_terminal(address)
  elseif entry.role=="prompt-routing-full-inventory-abort-presentation-skip" then rtx_abort_skip()
  elseif entry.role=="prompt-routing-done-redirect" then rtx_done_redirect()
  elseif entry.role=="prompt-routing-done-movem-stub-entry" then rtx_done_movem_stub()
  elseif entry.role=="prompt-routing-done-rts-stub-entry" then rtx_done_rts_stub()
  elseif entry.role=="prompt-routing-add-item-boundary" then rtx_add_item()
  elseif entry.role=="prompt-routing-case-result" then finish_routing_case(entry.index)
...
local function register_exec(address,role,index)
  if not callbacks[address] then
    callbacks[address]={};event_ids[#event_ids+1]=event.on_bus_exec(function()
      if observer_failed then return end
      local ok,message=pcall(function() current_pc=address;if not dispatch_equip_shared_update_effective_return(address) and not dispatch_joint_terminal_done_dispatcher(address) then for _,entry in ipairs(callbacks[address]) do dispatch(address,entry) end end end)
...
write_probe=function()
  ...
  register_exec(p.runtimeStartAddress,"prompt-routing-selection-loop-entry",0);register_exec(p.memberList.callAddress,"prompt-routing-member-list-controlled-service-call-shim",0);register_exec(precommit_state.serviceStub,"prompt-routing-generated-service-stub",0);register_exec(p.memberList.returnAddress,"prompt-routing-member-list-original-return",0);register_exec(p.memberCancelCompareAddress,"prompt-routing-member-cancel-compare",0);register_exec(p.memberCancelBranchAddress,"prompt-routing-member-cancel-branch",0);register_exec(p.heldItems.callAddress,"prompt-routing-held-items-controlled-service-call-shim",0);register_exec(p.heldItems.returnAddress,"prompt-routing-held-items-original-return",0);register_exec(p.capacityCompareAddress,"prompt-routing-capacity-compare",0);register_exec(p.capacityBranchAddress,"prompt-routing-capacity-branch",0);register_exec(p.equipmentType.callAddress,"prompt-routing-equipment-type-controlled-service-call-shim",0);register_exec(p.equipmentType.returnAddress,"prompt-routing-equipment-type-original-return",0);register_exec(p.equipmentTypeCompareAddress,"prompt-routing-equipment-type-compare",0);register_exec(p.toolAdmissionBranchAddress,"prompt-routing-tool-admission-branch",0);register_exec(p.equippability.callAddress,"prompt-routing-equippability-controlled-service-call-shim",0);register_exec(p.equippability.returnAddress,"prompt-routing-equippability-original-return",0);register_exec(p.equippabilityBranchAddress,"prompt-routing-equippability-branch",0);register_exec(p.addItemEntryAddress,"prompt-routing-add-item-boundary",0);register_exec(p.promptRouting.fullInventory.promptCallAddress-4,"prompt-routing-full-inventory-boundary-jsr",0);register_exec(p.promptRouting.nonEquippable.promptCallAddress-4,"prompt-routing-non-equippable-boundary-jsr",0);register_exec(precommit_state.terminalStub,"prompt-routing-dispatch-stub",0);register_exec(p.promptRouting.controlledResultStub.address,"prompt-routing-controlled-result-stub-entry",0);register_exec(p.promptRouting.controlledResultStub.jmpAddress,"prompt-routing-controlled-result-stub-jmp-boundary",0);register_exec(p.promptRouting.fullInventory.compareAddress,"prompt-routing-full-inventory-compare",0);register_exec(p.promptRouting.fullInventory.retryBranchAddress,"prompt-routing-full-inventory-branch",0);register_exec(p.promptRouting.nonEquippable.compareAddress,"prompt-routing-non-equippable-compare",0);register_exec(p.promptRouting.nonEquippable.retryBranchAddress,"prompt-routing-non-equippable-branch",0);register_exec(p.terminalShims[1].boundaryAddress,"prompt-routing-recipient-cancel-terminal",0);register_exec(p.fullInventoryAbortPresentationSkip.boundaryAddress,"prompt-routing-full-inventory-abort-presentation-skip",0);register_exec(p.promptRouting.doneNeutralBoundary.jointRedirect.address,"prompt-routing-done-redirect",0);register_exec(p.promptRouting.doneNeutralBoundary.jointRedirect.dispatcher.address,"prompt-routing-joint-terminal-done-dispatcher",0);register_exec(p.promptRouting.doneNeutralBoundary.movemStub.address,"prompt-routing-done-movem-stub-entry",0);register_exec(p.promptRouting.doneNeutralBoundary.rtsStub.address,"prompt-routing-done-rts-stub-entry",0)
...
while true do
  ...
  if rtx.active then
    rtx.frameCount=rtx.frameCount+1
    if rtx.frameCount>rtx.frameBudget then
      set_expectation("prompt-routing-watchdog","prompt-routing-watchdog-timeout",emu.getregister("M68K PC"),rtx.pendingService and rtx.pendingService.callPc or nil,rtx.pendingService and rtx.pendingService.targetPc or nil,rtx.pendingService and rtx.pendingService.returnPc or nil)
      fail_callback("prompt-routing case frame budget exhausted before route terminal")
    end
  end
  if ed.active then
