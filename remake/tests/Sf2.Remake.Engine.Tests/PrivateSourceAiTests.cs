...
public sealed class PrivateSourceAiTests
{
    ...
    public void ActualPrivateCommandsRunInactiveStandbyThenActivationAndSetSevenToNextPlayer()
    {
        ...
        Stay(session); var relay = Stay(session);
        ...
        Accept(session, new Move(ExplorationDirection.North)); Stay(session); // Real first Bowie move to (8,17).
        for (int steps = 0; session.Current.Battle.Round < 3; steps++)
        {
            Assert.True(steps < 6);
            if (session.Current.Selection!.Actor == new ActorRef("ally-0"))
            {
                for (int i = 0; i < 3; i++) Accept(session, new Move(ExplorationDirection.East));
                for (int i = 0; i < 2; i++) Accept(session, new Move(ExplorationDirection.North));
            }
            Stay(session);
        }
        ...
        Stay(session); Stay(session); var setSeven = Stay(session); // Chester, Bowie, Sarah; source enemies run in between.
