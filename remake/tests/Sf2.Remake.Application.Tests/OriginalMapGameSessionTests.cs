...
public sealed class OriginalMapGameSessionTests
{
    ...
    {
        var session = Start(Definition(EmptyWords()));
        ...
        var snapshot = new PrivateOriginalMapSessionSnapshot(before.Definition, before.Receipt, layout,
            5, position, runtime.Traversal.TryMove(layout,
            ...
            messengerAcceptance: before.MessengerAcceptance, castleGate: missing == "castle" ? null : PrivateOriginalMapCastleGateState.Completed(before.Definition.CastleGate!),
            currentRuntime: runtime, palaceFirstVisit: missing == "palace" ? null : new PrivateOriginalMapPalaceFirstVisitReceipt(before.Definition.PalaceFirstVisit!, 2),
            astralAcceptance: missing is "astral" or "palace" ? null : new PrivateOriginalMapAstralAcceptanceState(before.Definition.AstralAcceptance!, 3),
            // Dependent receipts cannot survive a missing prerequisite under the snapshot contract.
            middleTowerGuard: guard && missing is null ? new PrivateOriginalMapMiddleTowerGuardReceipt(before.Definition.MiddleTowerGuard!, 4) : null);
