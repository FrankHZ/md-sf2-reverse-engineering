...
public sealed class OriginalMapGameSessionTests
{
    ...
    {
        GameSession session = Start(Definition(EmptyWords()));
        LoadPostOpeningReference(session);
    ...
    [InlineData(true)]
    [InlineData(false)]
    public void WestTowerEntryRetainsControlledRouteAndRelocatesBothMovementEntrypoints(bool directCommand)
    {
        GameSession session = ReachAstral(faceActor: true);
        ...
        var bridge = session.PrivateOriginalMapBattleBridge;
        Assert.Equal(new MapPosition(16, 5), released.PlayerPosition);
        FinishMove(session, ExplorationDirection.North);
        for (int input = 0; input < 11; input++) FinishMove(session, ExplorationDirection.West);
        FinishMove(session, ExplorationDirection.North);
        Assert.Equal(new MapPosition(5, 3), session.PrivateOriginalMapSnapshot.PlayerPosition);
        var result = directCommand
            ? session.ApplyPrivateOriginalMap(new MoveExplorationCommand(ExplorationDirection.East))
            : FinishMove(session, ExplorationDirection.East).Move;
        var after = result.Snapshot;
        var receipt = Assert.IsType<PrivateOriginalMapCrossMapTransitionReceipt>(result.CrossMapTransition);
        Assert.Equal(OriginalMapRuntimeAdmission.WestTowerMap20TransitionCapability, receipt.Capability);
        Assert.Equal(new OriginalMapCrossMapTransitionIdentity(ContentProfile.PrivateLocal,
            new MapId("map19"), "Map19s6_WarpEvents", 1), receipt.RecordIdentity);
        Assert.Equal(new MapPosition(5, 3), receipt.Source);
        Assert.Equal(new MapPosition(6, 2), receipt.Trigger);
        Assert.Equal(new MapId("map20"), after.Map);
        Assert.Equal(new MapPosition(6, 37), after.PlayerPosition);
        Assert.Equal(new OriginalMapAreaRecordIdentity("Map20s2_Areas", 2), after.CurrentAreaDefinition.Identity);
        Assert.Equal(released.SimulationStep + 14, after.SimulationStep);
        Assert.Same(after.Definition.RuntimeCatalog.Resolve(after.Map), after.CurrentRuntime);
        Assert.Same(after.CurrentRuntime.WorkingLayout, after.WorkingLayout);
        ...
        Assert.Equal(PrivateOriginalMapPlayerLocomotionPhase.Relocated, session.PrivateOriginalMapPlayerLocomotion.Phase);
        Assert.False(session.PrivateOriginalMapPlayerLocomotion.IsMoving);
        Assert.Same(released.PalaceFirstVisit, after.PalaceFirstVisit);
        Assert.Same(released.AstralAcceptance, after.AstralAcceptance);
        Assert.True(after.PalaceFirstVisit!.CompletionFlag605Set);
        Assert.True(after.AstralAcceptance!.HandlerFlag607Set && after.AstralAcceptance.ProgramFlag608Set);
        Assert.Same(released.Receipt, after.Receipt);
        Assert.Same(released.Zone601, after.Zone601);
        Assert.Same(released.Sarah, after.Sarah);
        Assert.Same(released.Entity142, after.Entity142);
        Assert.Same(released.MessengerAcceptance, after.MessengerAcceptance);
        Assert.Same(released.CastleGate, after.CastleGate);
        Assert.Same(bridge, session.PrivateOriginalMapBattleBridge);
        Assert.Null(after.LastTraversal);
        Assert.Null(after.LastLayoutMutation);
        ...
        Assert.Null(session.PrivateOriginalMapSnapshot.LastCrossMapTransition);
        Assert.Same(after.AstralAcceptance, session.PrivateOriginalMapSnapshot.AstralAcceptance);
        var restarted = Start(released.Definition).PrivateOriginalMapSnapshot;
    ...
    [InlineData("approach")]
    [InlineData("direction")]
    public void WestTowerEntryDoesNotRelocateOutsideItsControlledBoundary(string scenario)
    {
        GameSession session = ReachAstral(faceActor: true);
        if (scenario is not "astral" and not "palace")
            Assert.IsType<PrivateOriginalMapAstralAcceptanceApplied>(
                session.RequestPrivateOriginalMapInteraction(session.PrivateOriginalMapSnapshot.SimulationStep));
        var before = session.PrivateOriginalMapSnapshot;
        var runtime = before.CurrentRuntime;
        MapPosition position = new(5, scenario == "approach" ? 4 : 3);
        var seeded = new PrivateOriginalMapSessionSnapshot(before.Definition, before.Receipt, runtime.WorkingLayout,
            ...
            astralAcceptance: before.AstralAcceptance);
        // A bounded seed isolates missing prerequisites from the earlier route that establishes them.
        typeof(GameSession).GetField("_privateOriginalMapSnapshot",
            System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.NonPublic)!.SetValue(session, seeded);
        var bridge = session.PrivateOriginalMapBattleBridge;
        var result = FinishMove(session, scenario == "direction" ? ExplorationDirection.North : ExplorationDirection.East);
        Assert.Null(result.Move.CrossMapTransition);
        Assert.Equal(new MapId("map19"), result.Move.Snapshot.Map);
        Assert.Same(seeded.PalaceFirstVisit, result.Move.Snapshot.PalaceFirstVisit);
        Assert.Same(seeded.AstralAcceptance, result.Move.Snapshot.AstralAcceptance);
        Assert.Same(bridge, session.PrivateOriginalMapBattleBridge);
    }

    [Fact]
    public void WestTowerEntryBusyRejectsBothMovementEntrypointsWithoutMutation()
    {
        GameSession session = ReachAstral(faceActor: true);
        Assert.IsType<PrivateOriginalMapAstralAcceptanceApplied>(
            session.RequestPrivateOriginalMapInteraction(session.PrivateOriginalMapSnapshot.SimulationStep));
        ...
        var animation = session.PrivateOriginalMapPlayerLocomotion;
        var bridge = session.PrivateOriginalMapBattleBridge;
        Assert.Equal(new MapPosition(5, 3), before.PlayerPosition);
        Assert.True(animation.IsMoving);
        Assert.Throws<InvalidOperationException>(() => session.ApplyPrivateOriginalMap(new(ExplorationDirection.East)));
        Assert.Throws<InvalidOperationException>(() => session.BeginPrivateOriginalMapPlayerLocomotion(new(ExplorationDirection.East)));
        Assert.Same(before, session.PrivateOriginalMapSnapshot);
        Assert.Same(animation, session.PrivateOriginalMapPlayerLocomotion);
        Assert.Same(bridge, session.PrivateOriginalMapBattleBridge);
    ...
    public void WestTowerEntryRequiresItsOwnAdmittedIdentityAndFacing(string drift)
    {
        AssertRejectedDefinition(Definition(EmptyWords(), omitWestTower: drift == "missing",
    ...
    public void PalaceFirstVisitExplicitlyCommitsOnceAndRetainsItsControlledResult()
    {
        GameSession session = ReachPalaceEntry();
        PrivateOriginalMapSessionSnapshot before = session.PrivateOriginalMapSnapshot;
        Assert.Null(before.PalaceFirstVisit);
        Assert.Equal(new MapPosition(23, 37), before.PlayerPosition);
        var bridge = session.PrivateOriginalMapBattleBridge;
        var animation = session.PrivateOriginalMapPlayerLocomotion;

        var applied = Assert.IsType<PrivateOriginalMapPalaceFirstVisitApplied>(
            session.RequestPrivateOriginalMapInteraction(before.SimulationStep));
        PrivateOriginalMapSessionSnapshot after = applied.Snapshot;
        Assert.Equal(before.SimulationStep + 1, after.SimulationStep);
        Assert.Equal(new MapPosition(23, 39), after.PlayerPosition);
        Assert.Same(applied.Receipt, after.PalaceFirstVisit);
        Assert.Equal(OriginalMapPalaceFirstVisitPreset.ControlledClear605And507, applied.Receipt.Preset);
        Assert.True(applied.Receipt.CompletionFlag605Set);
        Assert.True(applied.Receipt.Entity130Hidden);
        Assert.Equal(new MapPosition(20, 39), applied.Receipt.Entity131Endpoint);
        Assert.Same(before.CurrentRuntime, after.CurrentRuntime);
        Assert.Same(before.WorkingLayout, after.WorkingLayout);
        Assert.Same(before.EntityPopulation, after.EntityPopulation);
        Assert.Same(before.Receipt, after.Receipt);
        Assert.Same(before.Zone601, after.Zone601);
        Assert.Same(before.Sarah, after.Sarah);
        Assert.Same(before.Entity142, after.Entity142);
        ...
        Assert.False(session.PrivateOriginalMapPlayerLocomotion.IsMoving);
        Assert.Equal(animation.StoredCounter, session.PrivateOriginalMapPlayerLocomotion.StoredCounter);
        Assert.Equal(animation.SelectedHalf, session.PrivateOriginalMapPlayerLocomotion.SelectedHalf);
        AssertPalaceRejected(session, after.SimulationStep,
            OriginalMapPalaceFirstVisitPreset.ControlledClear605And507,
            PrivateOriginalMapPalaceFirstVisitFailureCode.AlreadyCompleted);

        Move(session, ExplorationDirection.South, 1);
        Assert.Equal(new MapPosition(23, 40), session.PrivateOriginalMapSnapshot.PlayerPosition);
        Assert.Same(applied.Receipt, session.PrivateOriginalMapSnapshot.PalaceFirstVisit);
        Assert.Null(Start(Definition(EmptyWords())).PrivateOriginalMapSnapshot.PalaceFirstVisit);
    }

    [Theory]
    [InlineData("stale", PrivateOriginalMapPalaceFirstVisitFailureCode.StaleSimulationStep)]
    [InlineData("preset", PrivateOriginalMapPalaceFirstVisitFailureCode.InvalidPreset)]
    [InlineData("position", PrivateOriginalMapPalaceFirstVisitFailureCode.EntryMismatch)]
    [InlineData("moving", PrivateOriginalMapPalaceFirstVisitFailureCode.LocomotionBusy)]
    [InlineData("map", PrivateOriginalMapPalaceFirstVisitFailureCode.EntryMismatch)]
    public void PalaceFirstVisitRejectionsPreserveSnapshotLocomotionAndBridge(
        string scenario, PrivateOriginalMapPalaceFirstVisitFailureCode code)
    {
        GameSession session = scenario == "map" ? Start(Definition(EmptyWords())) : ReachPalaceEntry();
        if (scenario == "position") Move(session, ExplorationDirection.South, 1);
        if (scenario == "moving")
        {
            session.BeginPrivateOriginalMapPlayerLocomotion(new MoveExplorationCommand(ExplorationDirection.South));
            Assert.True(session.PrivateOriginalMapPlayerLocomotion.IsMoving);
        }

        AssertPalaceRejected(session,
            session.PrivateOriginalMapSnapshot.SimulationStep + (scenario == "stale" ? 1 : 0),
            scenario == "preset" ? (OriginalMapPalaceFirstVisitPreset)99 : OriginalMapPalaceFirstVisitPreset.ControlledClear605And507,
            code);
    }
    ...
    [InlineData(true)]
    [InlineData(false)]
    public void PalaceFirstVisitAdmissionRequiresExactSourceBinding(bool missing)
    {
        AssertRejectedDefinition(Definition(EmptyWords(), omitPalaceFirstVisit: missing,
            palaceFirstVisit: new OriginalMapPalaceFirstVisitDefinition(
                OriginalMapRuntimeAdmission.PalaceInitBodySha256, new string('A', 64))),
            OriginalMapImportFailureCode.InvalidMapProjection);
    }

    private static void AssertPalaceRejected(GameSession session, long expectedStep,
        OriginalMapPalaceFirstVisitPreset preset, PrivateOriginalMapPalaceFirstVisitFailureCode code)
    {
        var snapshot = session.PrivateOriginalMapSnapshot;
        var animation = session.PrivateOriginalMapPlayerLocomotion;
        var bridge = session.PrivateOriginalMapBattleBridge;
        var rejected = Assert.IsType<PrivateOriginalMapPalaceFirstVisitRejected>(
            session.CompletePrivateOriginalMapPalaceFirstVisit(new(expectedStep, preset)));
        Assert.Equal(code, rejected.Code);
    ...
    private static GameSession ReachPalaceEntry()
    {
        GameSession session = Start(Definition(EmptyWords()));
        LoadPostOpeningReference(session);
        MoveToCastleGateApproach(session);
        ...
        Assert.Equal(new MapId("map19"), session.PrivateOriginalMapSnapshot.Map);
        Move(session, ExplorationDirection.North, 1);
        Move(session, ExplorationDirection.East, 3);
        Move(session, ExplorationDirection.North, 16);
        Move(session, ExplorationDirection.West, 9);
        Move(session, ExplorationDirection.North, 5);
        Move(session, ExplorationDirection.East, 2);
        Move(session, ExplorationDirection.North, 1);
        session.BeginPrivateOriginalMapPlayerLocomotion(new MoveExplorationCommand(ExplorationDirection.East));
        Assert.Equal(new MapId("map20"), session.PrivateOriginalMapSnapshot.Map);
        return session;
    }

    [Fact]
    public void RoyalExitCommitsMap20BeforeInitAndPreservesRouteState()
    {
        GameSession session = Start(Definition(EmptyWords()));
        LoadPostOpeningReference(session);
        MoveToCastleGateApproach(session);
        ...
        Move(session, ExplorationDirection.North, 4);
        Assert.Equal(new MapId("map19"), session.PrivateOriginalMapSnapshot.Map);
        PrivateOriginalMapSessionSnapshot entry = session.PrivateOriginalMapSnapshot;

        Move(session, ExplorationDirection.North, 1);
        Move(session, ExplorationDirection.East, 3);
        Move(session, ExplorationDirection.North, 14);
        Assert.Equal(new MapPosition(29, 15), session.PrivateOriginalMapSnapshot.PlayerPosition);
        Assert.Same(entry.MessengerAcceptance, session.PrivateOriginalMapSnapshot.MessengerAcceptance);
        Move(session, ExplorationDirection.North, 2);
        Move(session, ExplorationDirection.West, 4);
        Assert.Equal(new MapPosition(25, 13), session.PrivateOriginalMapSnapshot.PlayerPosition);
        Assert.Same(entry.CastleGate, session.PrivateOriginalMapSnapshot.CastleGate);
        Move(session, ExplorationDirection.West, 5);
        Move(session, ExplorationDirection.North, 5);
        Move(session, ExplorationDirection.East, 2);
        Move(session, ExplorationDirection.North, 1);
        Assert.Equal(new MapPosition(22, 4), session.PrivateOriginalMapSnapshot.PlayerPosition);

        PrivateOriginalMapPlayerLocomotionStarted result = session.BeginPrivateOriginalMapPlayerLocomotion(
            new MoveExplorationCommand(ExplorationDirection.East));
        PrivateOriginalMapSessionSnapshot after = result.Move.Snapshot;
        PrivateOriginalMapCrossMapTransitionReceipt receipt = Assert.IsType<PrivateOriginalMapCrossMapTransitionReceipt>(result.Move.CrossMapTransition);
        ...
        Assert.Equal((byte)3, result.Animation.OpaqueFacing);
        ...
        PrivateOriginalMapMoveApplied moved = session.ApplyPrivateOriginalMap(new MoveExplorationCommand(ExplorationDirection.South));
        Assert.Equal(new MapPosition(23, 38), moved.Snapshot.PlayerPosition);
        Assert.Null(moved.Snapshot.LastCrossMapTransition);
        Assert.Same(after.CurrentRuntime, moved.Snapshot.CurrentRuntime);
        GameSession restarted = Start(Definition(EmptyWords()));
        Assert.Equal(new MapId("map3"), restarted.PrivateOriginalMapSnapshot.Map);
        Assert.False(restarted.PrivateOriginalMapSnapshot.CastleGate!.Opened);
    }
    ...
    [Fact]
    public void RoyalExitRequiresItsExactSourceAndDirection()
    {
        GameSession session = Start(Definition(EmptyWords()));
        LoadPostOpeningReference(session);
        MoveToCastleGateApproach(session);
        ...
        Move(session, ExplorationDirection.West, 3);
        Move(session, ExplorationDirection.North, 4);
        // Off-route navigation here is synthetic: it exercises command admission only.
        Move(session, ExplorationDirection.West, 4);
        Move(session, ExplorationDirection.North, 25);
        Assert.Equal(new MapPosition(22, 5), session.PrivateOriginalMapSnapshot.PlayerPosition);
        PrivateOriginalMapMoveApplied wrongSource = session.ApplyPrivateOriginalMap(new MoveExplorationCommand(ExplorationDirection.East));
        Assert.Null(wrongSource.CrossMapTransition);
        Move(session, ExplorationDirection.West, 1);
        Move(session, ExplorationDirection.North, 1);
        PrivateOriginalMapMoveApplied wrongDirection = session.ApplyPrivateOriginalMap(new MoveExplorationCommand(ExplorationDirection.North));
        Assert.Null(wrongDirection.CrossMapTransition);
        Assert.Equal(new MapId("map19"), wrongDirection.Snapshot.Map);
    }
    ...
    private static void LoadPostOpeningReference(GameSession session)
    {
        session.ApplyPrivateOriginalMapReferenceStart(PrivateOriginalMapReferenceStart.Read(
            Path.Combine(AppContext.BaseDirectory, "reference", "map3-post-opening-reference-start.json")));
        Assert.Null(session.PrivateOriginalMapSnapshot.LastMessengerAcceptance);
        Assert.True(session.PrivateOriginalMapSnapshot.MessengerAcceptance!.Accepted);
    }
    ...
    {
        var result = GameSession.StartPrivateOriginalMap(new AcceptedSource(Accepted(definition)), Request());
        Assert.True(result is PrivateOriginalMapGameSessionStarted,
            result is PrivateOriginalMapGameSessionStartRejected rejected
                ? $"{rejected.Diagnostic.Code}:{rejected.Diagnostic.Field}:{rejected.Diagnostic.Message}"
                : "Unexpected start result.");
        return Assert.IsType<PrivateOriginalMapGameSessionStarted>(result).Session;
    }
...
internal static class AcceptedOriginalMapRuntimeCatalog
{
    ...
    internal static OriginalMapCrossMapTransitionDefinition RoyalTransition() =>
        new(
            new OriginalMapCrossMapTransitionIdentity(
                ContentProfile.PrivateLocal,
                new MapId(OriginalMapRuntimeAdmission.Map19Id),
                OriginalMapRuntimeAdmission.RoyalMap20WarpResourceId,
                OriginalMapRuntimeAdmission.RoyalMap20WarpRecordOrdinal),
            OriginalMapRuntimeAdmission.RoyalMap20WarpSourceTriggerX,
            OriginalMapRuntimeAdmission.RoyalMap20WarpSourceTriggerY,
            new MapPosition(
                OriginalMapRuntimeAdmission.RoyalMap20WarpApproachX,
                OriginalMapRuntimeAdmission.RoyalMap20WarpApproachY),
            OriginalMapRuntimeAdmission.RoyalMap20WarpDirection,
            new MapPosition(
                OriginalMapRuntimeAdmission.RoyalMap20WarpTriggerX,
    ...
    private static OriginalMapExplorationRuntimeDefinition Map19Runtime()
    {
        ...
        // Only the source-shaped stair/zone facts needed by the accepted 38-input route.
        words[13 * WorkingMapLayout.ColumnCount + 24] = OriginalMapTraversal.LeftStairMask;
        words[12 * WorkingMapLayout.ColumnCount + 23] = OriginalMapTraversal.LeftStairMask;
        words[11 * WorkingMapLayout.ColumnCount + 22] = OriginalMapTraversal.LeftStairMask;
        words[10 * WorkingMapLayout.ColumnCount + 21] = OriginalMapTraversal.LeftStairMask;
        words[4 * WorkingMapLayout.ColumnCount + 22] = OriginalMapTraversal.RightStairMask;
        words[3 * WorkingMapLayout.ColumnCount + 23] = OriginalMapTraversal.RightStairMask | 0x1000;
        words[3 * WorkingMapLayout.ColumnCount + 5] = OriginalMapTraversal.RightStairMask;
        words[2 * WorkingMapLayout.ColumnCount + 6] = OriginalMapTraversal.RightStairMask | 0x1000;
        words[15 * WorkingMapLayout.ColumnCount + 29] = 0x1400;
        words[13 * WorkingMapLayout.ColumnCount + 25] = 0x1400;
    ...
    private static OriginalMapExplorationRuntimeDefinition Map20Runtime()
    {
        ...
        OriginalMapBlockCatalog blocks = new(
            Enumerable.Range(0, OriginalMapRuntimeAdmission.Map20BlockCount)
                .Select(index => new OriginalMapBlockDefinition(
                    new OriginalMapBlockRecordIdentity(
                        OriginalMapRuntimeAdmission.Map20BlocksetResourceId,
                        index),
                    new ushort[OriginalMapBlockDefinition.OpaqueWordCount])),
            OriginalMapRuntimeAdmission.Map20BlocksetProjectionDigest);
        OriginalMapAreaCatalog areas = new(
            new[] { (0, 0, 40, 30), (0, 33, 41, 45), (14, 47, 41, 60) }
                .Select((bounds, index) => new OriginalMapAreaDefinition(
                    new OriginalMapAreaRecordIdentity(OriginalMapRuntimeAdmission.Map20AreaResourceId, index + 1),
                    new OriginalMapTraversalArea(bounds.Item1, bounds.Item2, bounds.Item3, bounds.Item4),
                    new OriginalMapAreaWordPair(0, 0), new OriginalMapAreaWordPair(0, 0),
                    new OriginalMapAreaWordPair(256, 256), new OriginalMapAreaWordPair(256, 256),
                    new OriginalMapAreaBytePair(0, 0), new OriginalMapAreaBytePair(0, 0),
                    mainLayerType: 255, defaultMusic: 38)));
        OriginalMapEntityDefinition[] entities = Enumerable.Range(
                0,
                OriginalMapRuntimeAdmission.Map20EntityRecordCount)
            .Select(index => new OriginalMapEntityDefinition(
                new OriginalMapEntityRecordIdentity(
                    OriginalMapRuntimeAdmission.Map20EntityListResourceId,
                    index + 1),
                rawX: index switch { 2 => (byte)19, 3 => (byte)18, _ => checked((byte)(index + 1)) },
                rawY: index switch { 2 => (byte)39, 3 => (byte)40, 7 => (byte)2, _ => (byte)1 },
                opaqueFacing: 0,
                mapSprite: checked((byte)(index + 1)),
                index < OriginalMapRuntimeAdmission.Map20FixedEntityRecordCount
                    ? [0, 0, 0, 0]
                    : [0xFF, checked((byte)(index + 1)), 2, 1]))
            .ToArray();
        OriginalMapEntityPopulation population = new(
            map,
            new MapSetupId(OriginalMapRuntimeAdmission.Map20SelectedSetupId),
            entities,
            OriginalMapRuntimeAdmission.Map20EntityProjectionDigest);
        return new OriginalMapExplorationRuntimeDefinition(
            map,
            layout,
            blocks,
...
internal static class AcceptedOriginalMapPalaceFirstVisit
{
    internal static OriginalMapPalaceFirstVisitDefinition Create() => new(
