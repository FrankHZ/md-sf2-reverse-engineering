...
{
    private static bool SarahHealReview => System.Environment.GetEnvironmentVariable("SF2_BATTLE01_CONTROL_REVIEW") == "sarah-heal";
    private static bool FirstKillReview => SarahHealReview || System.Environment.GetEnvironmentVariable("SF2_BATTLE01_CONTROL_REVIEW") == "chester-first-kill";
    private static ushort? ExpectedChesterKills => FirstKillReview ? (ushort)0 : null;
    private static string ExpectedPartyId => SarahHealReview ? OriginalBattle01ControlledPartyPreset.SarahHealComparisonId : FirstKillReview ? OriginalBattle01ControlledPartyPreset.ChesterFirstKillComparisonId :
        OriginalBattle01ControlledPartyPreset.LeaderDefeatComparisonId;
    ...
    private async Task ReviewBattle01Control()
    {
        ...
        if (SarahHealReview)
            Require(Map3RuntimeProfileSelection.Parse(OS.GetCmdlineUserArgs()).SarahHealRequested &&
                Field<bool>(_root,"_sarahHealRequested") && _session.PrivateOriginalBattle01 is null,
                "The early Sarah HEAL option is bound before physical N");
        else if (FirstKillReview)
            Require(Map3RuntimeProfileSelection.Parse(OS.GetCmdlineUserArgs()).ChesterFirstKillRequested &&
                Field<bool>(_root,"_chesterFirstKillRequested") && _session.PrivateOriginalBattle01 is null,
                "The real launch opt-in is bound before physical N and initialization");
        ...
        Require(FirstKillReview
            ? ready!.Preparation.Party.Id == ExpectedPartyId && ready.Preparation.Party.Allies[2].CurrentKills == 0 &&
                ready.Battle.Roster[2].Stats.CurrentKills == 0 && ready.Preparation.ReturnInputs is null &&
                ready.Preparation.ArrivalInputs is null && ready.Battle.ReturnAdmission is null
            : ready!.Preparation.ReturnInputs == OriginalBattle01ControlledReturnInputs.GransealFirstAttemptComparison &&
                ready.Battle.ReturnAdmission is not null,
            "N retains the exact early comparison and its return/arrival selections");
        Require(ready!.Battle.BattleEntryFlag399,"N completed the controlled battle entry");
    ...
    {
        bool firstDefeat = System.Environment.GetEnvironmentVariable("SF2_BATTLE01_CONTROL_REVIEW") is "first-enemy-defeat" or "chester-enemy-hit" or "chester-player-attack" or "second-enemy-defeat" or "first-ally-defeat" or "leader-defeat-pending" or "chester-counterattack" or "chester-first-kill" or "sarah-heal";
        var presenter=Field<PrivateBattle01Presenter>(_root,"_privateBattle01Presenter");
        Require(ReferenceEquals(ready,_session.PrivateOriginalBattle01) && ready.Battle.Roster[0].Stats.HpCurrent==9 &&
            ...
            ready.Preparation.Party.Id==ExpectedPartyId,
    ...
    private async Task ReviewFirstEnemyDefeat(PrivateOriginalBattle01SessionSnapshot roundSeven)
    {
        ...
        if (System.Environment.GetEnvironmentVariable("SF2_BATTLE01_CONTROL_REVIEW") is "chester-enemy-hit" or "chester-player-attack" or "second-enemy-defeat" or "first-ally-defeat" or "leader-defeat-pending" or "chester-counterattack" or "chester-first-kill" or "sarah-heal")
        {
    ...
    private async Task ReviewChesterEnemyHit(PrivateOriginalBattle01SessionSnapshot receipt59)
    {
        ...
        Require(receipt.BeforeAfterTurn == new Battle01FactionCounts(3,5) && receipt.AfterAfterTurn == new Battle01FactionCounts(3,5) &&
            ...
            end.Roster[2].Stats.CurrentExp == 0 && end.Roster[2].Stats.CurrentKills == ExpectedChesterKills,
        ...
        if (System.Environment.GetEnvironmentVariable("SF2_BATTLE01_CONTROL_REVIEW") is "chester-player-attack" or "second-enemy-defeat" or "first-ally-defeat" or "leader-defeat-pending" or "chester-counterattack" or "chester-first-kill" or "sarah-heal")
        {
    ...
    private async Task ReviewChesterPlayerAttack(PrivateOriginalBattle01SessionSnapshot ready)
    {
        ...
        Require(ReferenceEquals(ready, _session.PrivateOriginalBattle01) &&
            ready.Preparation.Party.Id == ExpectedPartyId &&
            ready.Preparation.Party.Allies[2].CurrentExp == 0 && ready.Preparation.Party.Allies[2].CurrentKills == ExpectedChesterKills,
        ...
        Require(end.Roster.Select(u => u.Position).SequenceEqual(new MapPosition?[] {new(11,15),new(9,17),new(11,14),
            ...
            end.Roster[2].Stats.CurrentExp == 10 && end.Roster[2].Stats.CurrentKills == ExpectedChesterKills &&
        ...
        if (System.Environment.GetEnvironmentVariable("SF2_BATTLE01_CONTROL_REVIEW") is "second-enemy-defeat" or "first-ally-defeat" or "leader-defeat-pending" or "chester-counterattack" or "chester-first-kill" or "sarah-heal")
        {
    ...
    private async Task ReviewSecondEnemyDefeat(PrivateOriginalBattle01SessionSnapshot sarah)
    {
        ...
        if (System.Environment.GetEnvironmentVariable("SF2_BATTLE01_CONTROL_REVIEW") is "first-ally-defeat" or "leader-defeat-pending" or "chester-counterattack" or "chester-first-kill" or "sarah-heal")
        {
            if (System.Environment.GetEnvironmentVariable("SF2_BATTLE01_CONTROL_REVIEW") is "chester-counterattack" or "chester-first-kill" or "sarah-heal")
    ...
    private async Task ReviewChesterCounterattack(PrivateOriginalBattle01SessionSnapshot chester)
    {
        ...
        Require(ready.Battle.RandomSeedImage==0xF6711234 && ready.Battle.RandomSeedCopy==0x0134 &&
            ...
            ready.Battle.Roster[2].Stats.CurrentKills == ExpectedChesterKills,"Physical Sarah endpoint retains counter accounting and two prior corpses");
        ...
        if (FirstKillReview) { await ReviewChesterFirstKill(final); return; }
        File.WriteAllText(Path.Combine(_output,"receipt.json"),JsonSerializer.Serialize(new {
    ...
    private async Task ReviewChesterFirstKill(PrivateOriginalBattle01SessionSnapshot sarah)
    {
        var presenter = Field<PrivateBattle01Presenter>(_root,"_privateBattle01Presenter");
        var json = new JsonSerializerOptions { WriteIndented = true, MaxDepth = 256 };
        foreach (int actor in new[] {1,0})
        {
            Require(_session.PrivateOriginalBattle01!.Battle.FirstControl?.ActorIndex == actor,"Actual R11 origin choice");
            await PressBattleKey(Key.Space); await PressBattleKey(Key.Space);
        }
        var ready = _session.PrivateOriginalBattle01!;
        Require(ready.Battle.FirstRound!.RoundNumber == 12 && ready.Battle.FirstControl?.ActorIndex == 2 &&
            ready.Battle.RandomSeedImage == 0x44E81234 && ready.Battle.RandomSeedCopy == 0x0134 &&
            ReferenceEquals(sarah.Battle.TurnCompletion,ready.Battle.TurnCompletion!.Previous!.Previous),"Actual93 prefix reaches Chester with no extra STAY cycle");
        await CaptureControl("33-first-kill-r12-chester-ready");
        await PressBattleKey(Key.Space); await PressBattleKey(Key.A);
        for (int i = 0; presenter.Projection!.SelectedTargetIndex != 129; i++)
        { Require(i < 4,"Actual129 target is reachable"); await PressBattleKey(Key.L); }
        await CaptureControl("34-first-kill-target129");
        await PressBattleKey(Key.Backspace); await CaptureControl("35-first-kill-target-cancel");
        await PressBattleKey(Key.A);
        for (int i = 0; presenter.Projection!.SelectedTargetIndex != 129; i++)
        { Require(i < 4,"Actual129 reselected"); await PressBattleKey(Key.L); }
        var selected = _session.PrivateOriginalBattle01!;
        var copied = (PrivateOriginalBattle01SessionSnapshot)Activator.CreateInstance(typeof(PrivateOriginalBattle01SessionSnapshot),
            BindingFlags.Instance|BindingFlags.NonPublic,null,new object?[] {selected.Preparation,selected.Battle,selected.SourceLocomotion,selected.SourceBridge},null)!;
        ...
        try
        {
            Require(_session.ConfirmPrivateOriginalBattle01PlayerAttack(copied,2) is PrivateOriginalBattle01PlayerAttackApplied,"Copied selected kill publishes once");
            var after = _session.PrivateOriginalBattle01!; inspected = after.Battle;
            var receipt = inspected.TurnCompletion!; var d = receipt.PlayerPhysicalAttack!;
            Require(ReferenceEquals(receipt.Previous,selected.Battle.TurnCompletion) &&
                ReferenceEquals(receipt.Policy,Battle01PlayerPhysicalCompletionPolicy.ControlledChesterFirstKill) &&
                d.ActorIndex == 2 && d.TargetIndex == 129 && d.Effect.Damage == 3 && d.Effect.TemporaryHp == 0 &&
                inspected.Roster[2].Stats.HpCurrent == 1 && inspected.Roster[2].Stats.CurrentExp == 54 && inspected.Roster[2].Stats.CurrentKills == 1 &&
                inspected.Roster[0].Stats.CurrentExp == 63 && inspected.Roster[0].Stats.CurrentKills == 2 && inspected.CurrentGold == 180 &&
                inspected.Roster[4].Position is null && inspected.OccupantAt(new(10,4)) == -1 &&
                inspected.RandomSeedImage == 0x323E1234 && inspected.RandomSeedCopy == 0x0134 &&
                inspected.FirstRound!.CurrentCandidate?.CombatantIndex == 128,"Actual94 kill and separate accounting match the bounded source consequence");
            Require(d.Effect.Rolls.Select(r => r.Result).SequenceEqual(new ushort[] {3,7,0,0,6,3}) &&
                receipt.EnemyDefeat!.FirstWorklist.SequenceEqual(new[] {129}) && receipt.EnemyDefeat.AfterTurnWorklist.Count == 0 &&
                receipt.BeforeAfterTurn == new Battle01FactionCounts(3,3) && receipt.AfterAfterTurn == receipt.BeforeAfterTurn,"Actual94 roll/cleanup/count channels");
            presenter.Project(after.Battle,"TEST COPY: Chester first kill. Actual128 is next.");
            Require(presenter.Projection!.ChesterKills == 1 && presenter.Projection.BowieKills == 2 &&
                presenter.Projection.AllyStatus.Contains("A2 (9,4) HP 1 EXP 54") &&
                presenter.Projection.AttackResult!.Contains("Chester kills 0 -> 1") &&
        ...
        {
            typeof(GameSession).GetProperty(nameof(GameSession.PrivateOriginalBattle01))!.SetValue(_session,selected);
            presenter.Project(selected.Battle,"Restored exact selected target. Space confirms.");
        }
        await PressBattleKey(Key.Space);
        var final = _session.PrivateOriginalBattle01!;
        ...
        if (SarahHealReview || System.Environment.GetEnvironmentVariable("SF2_BATTLE01_SURVIVAL_REVIEW") == "1")
        {
    ...
    private async Task ReviewFiveSurvivorRound(PrivateOriginalBattle01SessionSnapshot bowie)
    {
        ...
        Require(ready.Battle.Roster[0].Position==new MapPosition(11,13) && ready.Battle.Roster[0].Stats.HpCurrent==3 &&
            ...
            ready.Battle.Roster[1].Stats.CurrentExp == (SarahHealReview ? (byte?)0 : null) && ready.Battle.Roster[1].Stats.CurrentKills is null && ready.Battle.Roster[1].Stats.CurrentDefeats is null &&
        ...
        Battle01PlayerPhysicalAttack.RequireAccountingInputs(final.Battle,0,0,0,0,0,0,SarahHealReview ? (byte?)0 : null);
        ...
        if (SarahHealReview) { await ReviewSarahHeal(final); return; }
        File.WriteAllText(Path.Combine(_output,"receipt.json"),JsonSerializer.Serialize(new{
    ...
    private async Task ReviewSarahHeal(PrivateOriginalBattle01SessionSnapshot ready)
    {
        var presenter = Field<PrivateBattle01Presenter>(_root, "_privateBattle01Presenter");
        var json = new JsonSerializerOptions { WriteIndented = true, MaxDepth = 256 };
        string frozen = JsonSerializer.Serialize(ready, json);
        Require(_initialBattle01!.Preparation.Party.Id == OriginalBattle01ControlledPartyPreset.SarahHealComparisonId &&
            _initialBattle01.Battle.Roster[1].Stats.CurrentExp == 0 && ready.Battle.Roster[1].Stats.CurrentExp == 0 &&
            ReferenceEquals(_initialBattle01.Preparation, ready.Preparation), "EXP0 comes from actual earliest preparation");
        await Step(Key.Space, PrivateBattle01Input.Confirm, "48-sarah-heal-action-choice");
        Require(presenter.Projection!.Controls.Contains("M: Magic") && !presenter.Projection.Controls.Contains("A: Attack"), "Only supported Sarah action is prompted");
        await Step(Key.M, PrivateBattle01Input.Magic, "49-sarah-heal-spell-choice");
        var spell = _session.PrivateOriginalBattle01!;
        await Step(Key.Space, PrivateBattle01Input.Confirm, "50-sarah-heal-bowie-target");
        Require(_session.PrivateOriginalBattle01!.Battle.FirstControl!.Movement.Healing!.Targets.SequenceEqual(new[]{0,1}) &&
            presenter.Projection!.SelectedTargetIndex == 0, "Bowie and self are the actual legal targets");
        await Step(Key.L, PrivateBattle01Input.East, "51-sarah-heal-self-unsupported");
        var unsupported = _session.PrivateOriginalBattle01!;
        await Step(Key.Space, PrivateBattle01Input.Confirm, "52-sarah-heal-self-rejected");
        Require(ReferenceEquals(unsupported,_session.PrivateOriginalBattle01) && !presenter.Projection!.Controls.Contains("Space"),
            "Range-legal self is typed Unsupported and cannot publish or suggest a cast");
        await Step(Key.Backspace, PrivateBattle01Input.Cancel, "53-sarah-heal-target-cancel");
        Require(JsonSerializer.Serialize(spell,json)==JsonSerializer.Serialize(_session.PrivateOriginalBattle01,json), "Target cancel restores whole spell choice");
        await Step(Key.Backspace, PrivateBattle01Input.Cancel, "54-sarah-heal-spell-cancel");
        await Step(Key.Backspace, PrivateBattle01Input.Cancel, "55-sarah-heal-movement-cancel");
        Require(frozen==JsonSerializer.Serialize(_session.PrivateOriginalBattle01,json), "Every cancellation layer restores exact102");
        await Step(Key.Space, PrivateBattle01Input.Confirm, "56-sarah-heal-action-reselected");
        await Step(Key.M, PrivateBattle01Input.Magic, "57-sarah-heal-spell-reselected");
        await Step(Key.Space, PrivateBattle01Input.Confirm, "58-sarah-heal-bowie-reselected");
        var selected=_session.PrivateOriginalBattle01!;
        var copy=Copy(selected); PrivateOriginalBattle01SessionSnapshot cast, expected;
        string boundary;
        typeof(GameSession).GetProperty(nameof(GameSession.PrivateOriginalBattle01))!.SetValue(_session,copy);
        try
        {
            cast=((PrivateOriginalBattle01PlayerHealingApplied)_session.ConfirmPrivateOriginalBattle01PlayerHealing(copy,1)).Snapshot;
            var r=cast.Battle.TurnCompletion!;
            Require(r.PlayerHealing is {ActorIndex:1,TargetIndex:0,Effect.Recovery:9,Effect.AwardedExp:17} &&
                r.PlayerPhysicalAttack is null && r.EnemyPhysicalAttack is null && ReferenceEquals(r.Previous,selected.Battle.TurnCompletion),
                "One distinct heal receipt preserves the actual102 history");
            Require(cast.Battle.Roster[1].Stats.MpCurrent==7 && cast.Battle.Roster[1].Stats.CurrentExp==17 &&
                cast.Battle.Roster[0].Stats.HpCurrent==12 && cast.Battle.RandomSeedImage==0x02A11234 && cast.Battle.RandomSeedCopy==0x0234,
                "Actual HEAL1 scalar and independent RNG channels agree");
            presenter.Project(cast,"TEST COPY: actual HEAL 1 commit before dispatcher.");
            await CaptureControl("59-sarah-heal-cast-test-copy","actual facade cast; production immediately dispatches");
            boundary=PrivateBattle01Ui.DispatchNext(_session,cast);expected=_session.PrivateOriginalBattle01!;
        }
        finally { typeof(GameSession).GetProperty(nameof(GameSession.PrivateOriginalBattle01))!.SetValue(_session,selected); presenter.Project(selected,"Bowie selected. Space casts HEAL 1."); }
        await PressBattleKey(Key.Space);
        var actual=_session.PrivateOriginalBattle01!;
        Require(JsonSerializer.Serialize(expected,json)==JsonSerializer.Serialize(actual,json) && presenter.Projection!.Status==boundary,
            "Physical cast and complete actual dispatch equal the whole facade snapshot");
        await CaptureControl("60-sarah-heal-actual-dispatch");
        if(actual.Battle.FirstControl is { } control)
        {
            // Derive a real reachable destination from this returned actor, without guessing it from the STAY probe.
            var origin=control.Movement.Cursor;
            var choices=new[]{(Key.I,PrivateBattle01Input.North,new MapPosition(origin.X,origin.Y-1)),
                (Key.L,PrivateBattle01Input.East,new MapPosition(origin.X+1,origin.Y)),
                (Key.K,PrivateBattle01Input.South,new MapPosition(origin.X,origin.Y+1)),
                (Key.J,PrivateBattle01Input.West,new MapPosition(origin.X-1,origin.Y))};
            var choice=choices.First(c=>control.Movement.Range.CanStopAt(c.Item3));
            await Step(choice.Item1,choice.Item2,"61-sarah-heal-next-player-preview");
            await Step(Key.Space,PrivateBattle01Input.Confirm,"62-sarah-heal-next-player-confirm");
            await Step(Key.Backspace,PrivateBattle01Input.Cancel,"63-sarah-heal-next-player-cancel");
            Require(JsonSerializer.Serialize(actual,json)==JsonSerializer.Serialize(_session.PrivateOriginalBattle01,json),"Actual next player cancel restores entire returned snapshot");
        }
        else Require(boundary.Contains("unavailable",StringComparison.OrdinalIgnoreCase) || boundary.Contains("rejected",StringComparison.OrdinalIgnoreCase),
        ...
        if (System.Environment.GetEnvironmentVariable("SF2_BATTLE01_SECOND_FIVE_SURVIVOR_REVIEW") == "1")
        {
            ...
            bool reviewBowieCounter = System.Environment.GetEnvironmentVariable("SF2_BATTLE01_BOWIE_COUNTER_REVIEW") == "1";
            PrivateOriginalBattle01SessionSnapshot completed105, generated14, completed106, endpoint;
            PrivateOriginalBattle01SessionSnapshot? counter107 = null;
            string continuation;
            ...
            try
            {
                ...
                Require(_session.CompletePrivateOriginalBattle01EnemyPursuit(generated14, 128) is PrivateOriginalBattle01AttackSelectionRequired,
                    "Actual128 reaches attack selection without publishing movement");
                completed106 = ((PrivateOriginalBattle01EnemyPhysicalAttackCompleted)_session.CompletePrivateOriginalBattle01EnemyPhysicalAttack(generated14, 128)).Snapshot;
                Require(completed106.Battle.FirstRound is { RoundNumber: 14, CurrentTurnOffset: 2 } && completed106.Battle.FirstControl is null &&
                    completed106.Battle.TurnCompletion is { CompletedActorIndex: 128, EnemyPhysicalAttack.TargetIndex: 0, EnemyPhysicalAttack.Effect.Damage: 3 } &&
                    ReferenceEquals(completed106.Battle.TurnCompletion.Previous, completed105.Battle.TurnCompletion) &&
                    completed106.Battle.RandomSeedImage == 0xAE581234 && completed106.Battle.RandomSeedCopy == 0x0034 && completed106.Battle.Roster[0].Stats.HpCurrent == 9,
                    "Actual facade128 completion retains the accepted106 pre-primary boundary");
                string beforeCounter = JsonSerializer.Serialize(completed106, json);
                presenter.Project(completed106, "TEST COPY: 128 completed; 133 counter pending.");
                await CaptureControl("71-second-five-survivor-106-test-copy", "actual facade boundary; production continues immediately");
                if (reviewBowieCounter)
                {
                    Require(_session.CompletePrivateOriginalBattle01EnemyPursuit(completed106, 133) is PrivateOriginalBattle01AttackSelectionRequired,
                        "Actual133 selects its physical branch without partial publication");
                    counter107 = ((PrivateOriginalBattle01EnemyPhysicalAttackCompleted)_session.CompletePrivateOriginalBattle01EnemyPhysicalAttack(completed106, 133)).Snapshot;
                    var receipt = counter107.Battle.TurnCompletion!; var attack = receipt.EnemyPhysicalAttack!; var counter = attack.Counterattack!;
                    Require(ReferenceEquals(receipt.Previous, completed106.Battle.TurnCompletion) && receipt.CompletedActorIndex == 133 &&
                        ReferenceEquals(receipt.Policy, Battle01PhysicalCompletionPolicy.ControlledNonlethalBowieCounterAndExp) &&
                        attack.TargetIndex == 0 && counter.Actor.Index == 0 && counter.Target.Index == 133 &&
                        attack.Effect.Damage == 3 && counter.Effect.Damage == 2 && counter.AwardedExp == 10 &&
                        attack.Effect.Reaction == new Battle01PhysicalReaction(0, -3, 0, 0, 1) &&
                        counter.Effect.Reaction == new Battle01PhysicalReaction(133, -2, 0, 0, 1) &&
                        counter.Actor.Stats.CurrentExp == 63 && counter.ActorAfterStats.CurrentExp == 73 &&
                        counter107.Battle.FirstControl is null && counter107.Battle.FirstRound is { RoundNumber: 14, CurrentTurnOffset: 4 } &&
                        counter107.Battle.RandomSeedImage == 0x33561234 && counter107.Battle.RandomSeedCopy == 0x0134,
                        "Actual single107 records ordered primary/counter HP, Bowie's10EXP and all14 main draws");
                    presenter.Project(counter107, "TEST COPY: 133 primary and Bowie counter completed in receipt107.");
                    Require(presenter.Projection!.AttackResult == "E5 -> A0: hit 3. HP 9 -> 6.\nCounter: A0 -> E5: hit 2. HP 5 -> 3. EXP +10: 63 -> 73.",
                        "Generic presenter retains both ordered HP effects and the actual allied EXP recipient");
                    await CaptureControl("72-bowie-counter-107-test-copy", "actual atomic facade result before player dispatch");
                }
                continuation = PrivateBattle01Ui.DispatchNext(_session, _session.PrivateOriginalBattle01!); endpoint = _session.PrivateOriginalBattle01!;
                Require(continuation == "Round 14. Player 0 ready." && endpoint.Battle.FirstControl?.ActorIndex == 0 &&
                    endpoint.Battle.FirstRound is { RoundNumber: 14, CurrentTurnOffset: 4 } &&
                    endpoint.Battle.Roster[0].Stats.HpCurrent == 6 && endpoint.Battle.Roster[0].Stats.CurrentExp == 73 &&
                    endpoint.Battle.Roster.Single(u => u.Index == 133).Stats.HpCurrent == 3 &&
                    endpoint.Battle.RandomSeedImage == 0x33561234 && endpoint.Battle.RandomSeedCopy == 0x0134,
                    "Unchanged dispatcher returns actual Bowie107 control after the observed counter");
                Require(beforeCounter == JsonSerializer.Serialize(completed106, json), "The complete pre-primary106 remains immutable");
            }
            ...
            Require(JsonSerializer.Serialize(endpoint, json) == JsonSerializer.Serialize(_session.PrivateOriginalBattle01, json) &&
                presenter.Projection!.Status == continuation && presenter.Projection.CanConfirm && presenter.Projection.Controls.Contains("Space"),
                "Physical STAY and whole automatic generation/dispatch match the API copy at actual Bowie control");
            await CaptureControl(reviewBowieCounter ? "73-bowie-counter-actual-ready" : "72-second-five-survivor-actual-ready");
            if (reviewBowieCounter)
            {
                var nextControl = endpoint.Battle.FirstControl!; var nextOrigin = nextControl.Movement.Cursor;
                var nextChoices = new[] { (Key.I, PrivateBattle01Input.North, new MapPosition(nextOrigin.X, nextOrigin.Y - 1)),
                    (Key.L, PrivateBattle01Input.East, new MapPosition(nextOrigin.X + 1, nextOrigin.Y)),
                    (Key.K, PrivateBattle01Input.South, new MapPosition(nextOrigin.X, nextOrigin.Y + 1)),
                    (Key.J, PrivateBattle01Input.West, new MapPosition(nextOrigin.X - 1, nextOrigin.Y)) };
                var nextChoice = nextChoices.First(c => nextControl.Movement.Range.CanStopAt(c.Item3));
                await Step(nextChoice.Item1, nextChoice.Item2, "74-bowie-counter-next-player-preview");
                await Step(Key.Space, PrivateBattle01Input.Confirm, "75-bowie-counter-next-player-confirm");
                await Step(Key.Backspace, PrivateBattle01Input.Cancel, "76-bowie-counter-next-player-cancel");
                Require(JsonSerializer.Serialize(endpoint, json) == JsonSerializer.Serialize(_session.PrivateOriginalBattle01, json),
                    "Physical movement/confirm/cancel restores the entire actual107 ready snapshot");
            }
            ...
            File.WriteAllText(Path.Combine(_output, "receipt.json"), JsonSerializer.Serialize(new {
                status = "Pass", scope = "actual early HEAL, second five-survivor generation, Bowie counter107 and actual usable Bowie control",
                ...
                completed106 = completed106.Battle, counter107 = counter107?.Battle,
                boundary = continuation, battle = endpoint.Battle, actualEarlyPreparation = true, exactCopiedAndPhysicalSnapshotMatch = true,
                complete104Cancellation = true, prePrimary106Immutable = true, whole107MovementCancellation = reviewBowieCounter, frames = _frames
            }, json));
            GD.Print($"SF2_BATTLE01_CONTROL_NATIVE_REVIEW Pass frames={_frames.Count} post-heal-bowie-counter boundary={continuation}");
            return;
        }
        File.WriteAllText(Path.Combine(_output,"receipt.json"),JsonSerializer.Serialize(new{
            ...
            actualEarlyPreparation=true,exactCopiedAndPhysicalSnapshotMatch=true,allCancellationLayers=true,frames=_frames
        },json));
        GD.Print($"SF2_BATTLE01_CONTROL_NATIVE_REVIEW Pass frames={_frames.Count} sarah-heal actualActor={actual.Battle.FirstControl?.ActorIndex} boundary={boundary}");
        ...
        async Task Step(Key key,PrivateBattle01Input input,string? frame)
        {
            ...
            typeof(GameSession).GetProperty(nameof(GameSession.PrivateOriginalBattle01))!.SetValue(_session,copy);
            try { status=PrivateBattle01Ui.Apply(_session,null,input);expected=JsonSerializer.Serialize(_session.PrivateOriginalBattle01,json); }
            finally { typeof(GameSession).GetProperty(nameof(GameSession.PrivateOriginalBattle01))!.SetValue(_session,before); }
            await PressBattleKey(key);
    ...
    private async Task ReviewFirstAllyDefeat(PrivateOriginalBattle01SessionSnapshot chester)
    {
        ...
        Require(ReferenceEquals(chester, _session.PrivateOriginalBattle01) &&
            chester.Preparation.Party.Id == ExpectedPartyId &&
            chester.Preparation.Party.Allies[2].CurrentDefeats == 0 &&
            chester.Battle.Roster[2].Stats.CurrentDefeats == 0 && chester.Battle.Roster[2].Stats.CurrentKills == ExpectedChesterKills,
        ...
        Require(end.FirstRound!.RoundNumber == 14 && end.FirstRound.CurrentTurnOffset == 0 &&
            ...
            end.Roster[1].Stats.HpCurrent == 11 && end.Roster[2].Stats.CurrentExp == 10 && end.Roster[2].Stats.CurrentKills == ExpectedChesterKills &&
    ...
    private async Task ReviewLeaderDefeatPending(PrivateOriginalBattle01SessionSnapshot sarah)
    {
        var presenter = Field<PrivateBattle01Presenter>(_root, "_privateBattle01Presenter");
        var json = new JsonSerializerOptions { MaxDepth = 256 };
        Require(sarah.Preparation.Party.Id == ExpectedPartyId &&
