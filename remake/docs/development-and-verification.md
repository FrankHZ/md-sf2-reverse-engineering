...
keep Bowie control after all six enemies and close input without regenerating a round.
...
or assert natural Map3 continuity, original scene fidelity or H3/H4 closure. For first-round
completion, use `first-round` after the owning managed gates. Four captures show Bowie ready,
provisional, cancelled and first-round exhausted. Require all nine linked receipts, offset18/FF,
all six enemy decisions and1483 generated bytes (1336 after128), final seed-copy0134 and own memories
14/34/24/24/24/24h, with mainA4991234 and all stats unchanged. The former `enemy-standby`,
`next-player` and `stay` names are aliases for this current four-frame chain. Do not replay earlier
modes solely for a new commit.
