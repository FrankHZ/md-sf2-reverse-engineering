...
the separate next-round API and bounded Godot dispatch continue only wholly inactive rounds.
...
The required selected-input chain proves real terrain/current-origin movement and reachable region1
rejection. Domain/Application cover wrong actor, stale/foreign/duplicate requests, receipt prefix,
