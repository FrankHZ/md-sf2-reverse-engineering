...
| Local presentation checkout/export preflight | repository-owned path-free descriptor, clean local-Git identity check, and atomic runtime-only exporter | **Implemented tooling prerequisite** | requires explicit commit/tree/manifest pins; asset repository remains read-only and currently contains no product manifest or product assets; no generator, rasterizer, Godot consumer, remote backup, or product fallback |
