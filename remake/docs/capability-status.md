...
| HUD SVG candidate derivation | pinned resvg archive/version, closed static-SVG policy, deterministic 2x/4x RGBA8 build, existing pack schema, and ignored-cache atomic publisher | **Implemented tooling prerequisite** | permits one exact untracked master over an otherwise clean checkout and changes ignored cache only; the real asset repository still has no accepted master/manifest/runtime asset, and no promotion, local commit, Godot consumer, or fidelity claim exists |
