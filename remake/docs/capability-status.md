...
| Local presentation asset-pack admission and first HUD preview | Application path-free typed catalog/receipt, Content-owned manifest/path/payload reader, explicit PrivateLocal profile values, and one Godot-only semantic catalog/preview node | **Implemented as a bounded local consumer** | exact semantic `hud.yes-no-window-frame`, accepted 2x/4x selection and one project-authored top-right chrome preview only; no Yes/No behavior, text, icon, Theme, generic window service, PCK asset, smoke marker, or fidelity claim |
