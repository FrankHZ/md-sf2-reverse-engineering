...
| Private Map 3 player initial-reference-frame consumer | fixed-root candidate builder, reviewed local source/master/runtime/manifest transaction, exact Content-owned lookup, and Godot-only logical-cell projection | **Implemented as bounded local consumer** | half zero replaces only the project-authored base-view player marker under explicit base-atlas selection; live admission visibility, animation counter, movement-facing timing, DMA/cache completion, standing/idle meaning, hardware/display/final-pixel parity, and redistribution remain unclaimed |
