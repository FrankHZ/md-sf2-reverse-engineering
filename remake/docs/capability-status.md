...
| Private Map 3 base-atlas diagnostic consumer | exact reviewed local asset transaction, explicit PrivateLocal base-view plus atlas selection, Content-owned payload recheck, and Godot-only physical-raster atlas projection | **Implemented as bounded local consumer** | retains the full selected 2x/4x raster in a physical ImageTexture while mapping it into the unchanged logical crop; current nearest replication remains exact, source/master never enter runtime or PCK, HUD opt-in remains independent, and no camera/layer/animation/hardware/display/final-pixel or fidelity claim is added |
