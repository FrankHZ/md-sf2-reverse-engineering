...
func remember(s: Dictionary, label: String) -> void:
    castle_records.append({"label":label,"map":s.get("map"),"player":player(s),"flags":s.get("flags"),"cursor":s.get("cursor"),"wait":s.get("wait"),"failure":s.get("failure"),"presentation":s.get("presentation")})
...
func finish(s: Dictionary) -> void:
    ...
    file.store_string(JSON.stringify({"issue":issue,"records":castle_records,"paletteMinimum":palette_min,"prioritySeen":priority_seen}, "  "))
    file.close()
