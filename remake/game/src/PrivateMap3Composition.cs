...
public sealed partial class Map3Root
{
        ...
        if (selection.IsAvailable)
        {
            plan = selection.PrivateBaseViewRequested
                ? PrivateMap3PresentationPlan.PrivateLocalWithBaseVisual(
                    ...
                    selection.PrivateCurrentAreaOverlayRequested)
        ...
        out PrivateLocalPresentationRasterMount? entity142Diagnostic)
    {
        ...
        if (selection.PrivateBaseAtlasRequested)
        {
            ...
            PrivateLocalPresentationAssetMountResult castleResult =
                catalog.MountCastleBaseAtlas(packRequest, acceptedPack, effectivePhysicalScale);
            if (castleResult is not PrivateLocalPresentationAssetMounted mountedCastle)
            {
                PrivateLocalPresentationAssetMountRejected rejected =
                    (PrivateLocalPresentationAssetMountRejected)castleResult;
                FailPrivateStartup(
                    $"PrivateLocal castle base atlas unavailable ({rejected.Diagnostic.Code}).",
                    selection.PrivateSmokeRequested,
                    "private-local");
                return false;
            }

            castleAtlas = mountedCastle.Asset;

            PrivateLocalPresentationAssetMountResult map21Result =
                catalog.MountMap21BaseAtlas(packRequest, acceptedPack, effectivePhysicalScale);
            if (map21Result is not PrivateLocalPresentationAssetMounted mountedMap21)
            {
                PrivateLocalPresentationAssetMountRejected rejected =
                    (PrivateLocalPresentationAssetMountRejected)map21Result;
                FailPrivateStartup(
                    $"PrivateLocal Map 21 base atlas unavailable ({rejected.Diagnostic.Code}).",
                    selection.PrivateSmokeRequested,
                    "private-local");
                return false;
            }
