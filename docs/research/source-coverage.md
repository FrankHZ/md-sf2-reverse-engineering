...
| Indexed findings | 121 | Confirmed symbol/table records in `manifests/research-index.json` |
...
| Address bindings | 495 | Checked ROM/RAM relationships between fixtures and symbols/state |
...
files and the corresponding `data` query yields 1,690. The index summary reports 121 records; its
verifier reports 53 H3 fixtures and 495 bindings. The default `uv run sf2 verify` checks those
...
Burst Rock exploder. The first concentrated H3 matrix confirms final action/target choice in one
launch. Static work now continues into the unused and remaining standby helpers while the
