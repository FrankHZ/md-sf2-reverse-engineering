...
The new `sf2-map3-castle-battle-unlock-static-v1` rail is static-only: it derives the 53-source
...
resumes through the primary loop, follow-up choices, explosion backedge, End, and the unentered
`ExecuteIndividualTurn` source return edge `0x24106`. It adds no H3 fixture: natural continuity,
...
The `sf2-map-event-scripted-transition-state-static-v1` rail closes the exact one-positive/913-zero
map-event target-program corpus for map-script/entity-action command families. Its sole positive is
Map 21's `$545B6..$54714` transition stream: 87 source/H1/ROM operation rows / 350 bytes, 27 command
definitions, and four payload contexts with one inherited context. Source/listing resolution retains
19 non-null definition-handler entries; public `retainedHandlers` contains 17 of those plus the two
map/entity dispatcher table joins. The 111 owned anchors are the 87 operation rows, 19
...
- **Partial:** battle AI has a complete source inventory plus static action-filter, attack-priority,
  ...
  The separate eleven-case Church Cure H3 rail enters original `ChurchMenu` and its original
  `@CheckCureAction` route for every record, then covers no-status, decline, one-below, exact-cost,
  and combined poison→stun→curse commits. It statically joins source, H1, and ROM for the three
  mutation seams, the status/equipment bits, fixed poison/stun prices, and Dark Sword's
  ...
  remains Unknown and priority-frozen. Its six-entry input surface now binds two-port raw sampling,
  state storage, wait-helper/source-call inventory, and a sixteen-case one-launch H3 matrix: direct
  `UpdatePlayerInputs` two-port sampling, direct `ApplyZ80BusUpdates` input-repeat behavior, and the
  four original wait helpers through their bounded `WaitForVInt`/enabled-VInt input-stage progression.
...
per-entity, per-area, or per-item H3 fixture queue.
...
119-row sprite-dialogue table by source/ROM provenance. Its one-launch 21-case H3 matrix now confirms
only handler-local entry/RTS, A6/stack, skip, direct-call/return, source-set call-register, and direct
state-write seams. Normal story reachability, rendered portrait/speech/controller timing, and unshimmed
service completion/persistence remain the three grouped `map-script-dialogue/*` questions.
...
Its 22-case one-launch H3 fixture now confirms only handler-local entry/return, cursor, direct
call/target/return, source-register, quake-write, slow-counter, and flash-loop records. External service
entries are shimmed after their target PCs execute. Four exact Unknown queues retain normal-story
reachability, visible palette/VDP/frame timing, unshimmed service completion/repeat/persistence, and
map/entity interactions; source names do not credit those outcomes as static coverage.
...
the two matching branches, and nine-target zero-inclusive direct/effective caller maps. Its one-launch,
ten-case H3 fixture confirms only handler-local entry/operand/branch/loop/callback/return records and the
two direct entity-byte-write seams. The remaining grouped H3 queue covers normal-story reachability,
player-visible output/timing/completion/repeat, bypassed-service effects including `WaitForVInt`, and
persistence/map-entity interactions; source names, fields, literals, tables, and callee names do not
credit any entity effect, selector meaning, visual, timing, persistence, or reachability behavior as
static coverage.
...
seven-target alias-aware direct/effective caller maps. Its one-launch 11-record H3 fixture additionally
observes the four full-H2-source-order inputs, busy direct return, controlled `$FFFF` comparison branch,
hide callback chronology, and all four menu selector partitions with A6/stack restoration. Service shims
record their actual entry role: direct effective-target shims execute for the wait/helper calls, while
jump-interface aliases execute only their instruction-target shims and return before their effective
targets execute. The handler's direct instruction PCs and callback return-resumption PCs still execute.
Thus the covered boundary is handler-local rather than UI/service behavior; service effects are
unobserved. The remaining grouped questions are normal-story reachability, full window/VDP timing, real
user choice/service side effects, and save/persistence/map-entity interactions.
...
persistence, or reachability behavior. Its one-launch nine-case H3 contract now observes only the H1
handler/RTS, A6 4/8 cursor boundary, exact source word pairs, two original lookup call/entry/return PC
triples, and the offset-18 byte seam plus two adjacent-byte sentinels. The original lookup body runs;
there is no lookup shim. The remaining neutral grouped H3 queue is
...
base/span/mask facts. Its retained ten rows are byte-semantic v1 projection guarded. The v2 fixture
adds six original-function `SaveGame`/poison/`LoadGame`/original-branch probes
for csc10 set slot 1, csc10 clear slot 2, csc11 flag 89 set/clear, csc13 flag 400, and csc13 word-wrap
flag 0. One failed attempt is non-evidence: premature COMBATANT_DATA seeding changed CURRENT_MAP and
bypassed the map-init call before the probe. One corrected BizHawk launch then completed all sixteen
rows through the wrapper-to-probe path: retained-v1 inputs use `$FF4004`–`$FF4009`, persistence streams
remain in the guarded `$FF4800`–`$FF4A00` gap, and scoped readback restored logical RAM, selected SRAM
domains, generated bytes, pointer, retained-v1 stream, prompt bytes, and zero callbacks. This is
synthetic in-process persistence evidence only. Normal story reachability, church/witch/service-menu and save-slot UI,
prompt presentation/input timing, map reload, copy/delete, cross-process/physical durability, power
loss/torn writes/corruption, VInt behavior, VDP/audio/rendering, service economics, and remake/design meaning
remain explicit Unknown/excluded questions.
...
adjacent direct-control inventory is 154 positive/760 zero program contexts, 205 contextual/201
physical direct-transfer PCs (143 returning calls and 62 tail jumps), 35 effective targets, 15
...
The adjacent dialogue-state inventory retains 24 positive/890 zero program contexts from the same
914-program / 3,579-operation mother corpus. Its five source-defined dialogue-state symbols have 100
contextual/72 physical direct-state PCs; 23 text-bearing plus one state-only program context have 89
contextual/76 physical text sites (69 numeric line references and 20 close sentinels), combining to
189 contextual/148 physical state-or-text PCs. It source/H1/ROM guards 414/374 operations, 72/64
labels, 17 table-owner joins, 22 source identities, and 379 anchors, while reaching-definition sets
remain static identities only. Actual reachability/path, lookup results, service/display/audio/timing,
state lifetime, and story meaning remain its grouped 15-question **Unknown** queue. Evidence date:
2026-08-25.
...
2026-08-25. The adjacent request-state inventory retains exactly 39 positive/875 zero contexts: 30
entity and nine zone programs selected by 45 source-defined writes to `CURRENT_SHOP_INDEX`,
`MAP_EVENT_TYPE`, `EGRESS_MAP`, `RAFT_MAP`, `RAFT_X`, or `RAFT_Y`. Its caller-local
...
91/2,968 counters were documentation debt rather than an unobserved baseline. The adjacent
item-transaction rail narrows the retained map-event corpus to eight contexts/seven physical programs,
15 contextual/13 physical item-service calls, nine contextual/seven physical result-predicate joins,
five item-event `d6` writes, and the FieldMenu/map-setup handoff. It is static-only: actual inventory,
service result, branch selection, consumption/acquisition, persistence, input, dialogue, audio,
presentation, and story meaning remain its grouped **Unknown** register. It raises H2 fixtures to 94
and address bindings to 3,007 without adding an H3 rail.
