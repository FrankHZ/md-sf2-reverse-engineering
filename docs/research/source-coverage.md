...
| Indexed findings | 111 | Confirmed symbol/table records in `manifests/research-index.json` |
...
| Address bindings | 485 | Checked ROM/RAM relationships between fixtures and symbols/state |
...
- **Partial:** battle AI has a complete source inventory plus static action-filter, attack-priority,
  ...
  and terrain obstruction/carving helpers,
  plus at least one indexed entry in every one of its 26 files; dispatcher/standby are parsed but
  deeper special-helper semantics remain open. Other battle systems still cover selected
...
Therefore 13.44% is the useful current code-file-reach snapshot, while whole-game semantic and remake
...
files and the corresponding `data` query yields 1,690. The index summary reports 111 records; its
verifier reports 53 H3 fixtures and 485 bindings. The default `uv run sf2 verify` checks those
...
terrain helpers and five Manhattan-ring tables. The first concentrated H3 matrix confirms final
action/target choice in one launch. Static work now continues into line-attacker/exploder and other special helpers while the
