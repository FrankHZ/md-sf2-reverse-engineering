...
- [`indexing.md`](./research/indexing.md)：机器可读的 symbol → ROM/RAM address → fixture → 文档/设计
  合同索引、校验规则和新增发现的落地流程。
- [`source-coverage.md`](./research/source-coverage.md)：当前源码覆盖分母、9.82% 可执行代码文件
  reach 的严格边界，以及静态优先、集中运行 H3 的子系统批处理节奏。
- [`battle-ai.md`](./research/battle-ai.md)：完整 battle AI 源码 inventory、action filter、攻击与
  治疗优先级/等级决策，以及待集中执行的 H3 问题矩阵。
