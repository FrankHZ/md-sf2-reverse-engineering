...
**未知或排除：** 负、越界、回绕或其他畸形索引；截断行；缺失长度字节；无效指针；跨地址空间行为；调用方验证；运行时可达性与结果使用；所述 `d0.w` 保存/恢复之外的调用方可视 `d1`、CCR 或寄存器行为；文本编码；字形选择；本地化；渲染；窗口布局；时序；持久性；以及替换内容策略。
...
已接受证据在已接受 ROM 身份下解析代表 H1 条目与固定源时间线。它不建立完整函数体的逐字节 H1/ROM 指令 parity。未来私有指令字节比较会是更强证据，不是当前 H4 要求。
...
H4 不得要求公开 fixture 中有原版字符串或表。它用合成行测试已接受关系，并在原版兼容输入可用时用私有 parity 适配器。
...
| 合同区域 | 证据标签 | 所有者 | 剩余边界 |
| --- | --- | --- | --- |
| `GetClassName` 身份/地址与 `findname.asm` 代表源 | **已确认静态** | `sf2-common-stats-static-v1` | 完整函数体字节 parity |
