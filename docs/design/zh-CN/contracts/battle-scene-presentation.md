...
- **未知原版行为：** 精确命令与帧时序、VInt/VDP 效果、调色板过渡外观、可见层组合与放置、512 字节召唤转移尾、每个选择器/索引组合的自然可达性，以及渲染帧 parity。
...
规范法术、召唤、状态动画与过渡记录由[Battle Effect Graphics Data](../../contracts/battle-effect-graphics-data.md)拥有。本合同不独立拥有或重新验证其私有容器、调色板、偏移、流、压缩、解码或 parity 身份。
