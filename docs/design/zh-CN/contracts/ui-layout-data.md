...
- **已确认原版结构：** 完整组装 vanilla UI 布局语料、有序法术等级指针路由、菱形边框变体、直接瓦片资源、source/H1/ROM 一致性，以及下文描述的不重叠覆盖地址核算。
- **推断原版行为：** 此处不提升任何内容。
...
> 本文件是 [`ui-layout-data.md`](../../contracts/ui-layout-data.md) 的中文镜像。英文原文始终是审阅基线；本镜像为派生文档，遵循 [`glossary.md`](../../glossary.md) 的术语规则（R1–R7）。证据标签按 R1 使用固定中文译法；源码标识符、fixture ID 与路径按 R2 原样保留。

## 合同边界

本合同定义原版 vanilla 构建组装的 UI 布局的静态身份与无损私有导入边界。它拥有：

1. 19 个组装源所有者及其 5,116 字节 source/H1/ROM 一致性边界；
...
审计检查了 fixture、验证器、源所有者文章、source/H1/ROM 一致性字段、全部 36 个 fixture 表绑定与当前研究索引。它发现那些绑定与 36 条当前未关联记录之间的精确双射：19 条 `auxiliary.data.static` 记录与 17 条 `ui.layout.static` 记录。注册刻意推迟到初步语义接受；本文档不改变索引。
...
- 两个排除源文件在组装 vanilla 一致性语料之外。其排除不是它们未使用、运行时不可达或在每个 alternate 构建中不可达的证据。
...
全部 19 个组装源与全部四个资源载荷在已接受所有者中有 source/H1/ROM 一致性。这闭合其静态存储身份，而非运行时使用。
...
菱形、魔法与物品菜单网格均为 18×6（108 个字）。该形状并非其字节计数的任意因数分解：每个所属菜单都加载 `$1206`，然后才发出其唯一一次 `CreateWindow` 请求；`CreateWindow` 将高字节用作宽度、低字节用作高度，同一源码函数随后复制其对应的 216 字节布局。除 source/H1/ROM 一致性外，验证器还检查全部三个消费者关系。
...
**已确认静态：** 指针表占用 64 字节并包含 16 个有序 longword 条目。其源符号是 `pt_layouts_SpellLevelIndicator`，其 ROM 地址是 `0x110A4`（69,796），所有者证明该表 source/H1/ROM 一致性。规范私有导入独立于其引用的布局保留该身份、地址、大小、源与 ROM hash 与 H1 溯源。
...
它们不获得借用 vanilla 地址或一致性声称。如果私有源检出包含它们，导入器应把它们保留为显式语料外溯源记录。未来证据可能建立 alternate 构建或运行时角色；在那之前，刻意使用、构建选择与可达性为 **未知**。
...
6. 两个排除源保持显式在组装 vanilla 一致性语料之外，不发明未使用或不可达状态；
7. 公开 fixture 与报告只暴露元数据、hash 与合成示例，绝不暴露原版布局网格、原始指针表字节或瓦片载荷；
8. 运行时渲染、本地化、可访问性与刻意呈现变更与静态原版数据一致性分别测试与报告。
...
| --- | --- | --- | --- |
| 19 个源所有者、5,116 组装字节、27 个网格、2,394 字、形状、属性与一致性 | **已确认静态** | `sf2-ui-layout-static-v1`（[`ui-layout-static-v1.json`](../../../../tests/fixtures/h2/ui-layout-static-v1.json)） | 运行时选择、变更、渲染与调用方可达性 |
...
| 排除窗口边框聚合与战斗机迷你状态 alternate | **已确认从 vanilla 组装一致性语料排除** | `sf2-ui-layout-static-v1`（[`ui-layout-static-v1.json`](../../../../tests/fixtures/h2/ui-layout-static-v1.json)） | alternate 构建意图、使用与可达性保持 **未知** |
