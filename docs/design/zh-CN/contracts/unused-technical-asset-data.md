...
- **已确认原版结构：** 一个含四个有序 Stack 流的 5,694 字节源容器、四个不同 8,192 字节解码结果、两个有序 16 色调色板记录、一个调色板指针、精确 source/H1/ROM 一致性，以及下文描述的受限注释剥离符号引用清单。
...
> 本文件是 [`unused-technical-asset-data.md`](../../contracts/unused-technical-asset-data.md) 的中文镜像。英文原文始终是审阅基线；本镜像为派生文档，遵循 [`glossary.md`](../../glossary.md) 的术语规则（R1–R7）。证据标签按 R1 使用固定中文译法；源码标识符、fixture ID 与路径按 R2 原样保留。
...
| **总 source/ROM 一致性** | **5,762** | 全部三个存储组件 |
...
导入器可以使用独立实现解码器、预解码私有数据或把四个结果变换为现代私有资源形式。保真要求已接受容器身份、有序边界、四个解码字节结果与一致性关系保持独立可验证。它不要求原版 Stack 命令解析器、位读取器、复制循环、历史表示、寄存器分配或指令顺序。
...
源名不证明哪个解码流、帧、表面或画面会使用任一调色板。指针身份与一致性不证明运行时解引用。
