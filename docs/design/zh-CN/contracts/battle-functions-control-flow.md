...
**未知或排除：** 精确输入节奏、重复、同时性与平台映射；完整调用方状态；目标列表合法性与构建；AI 打分与选择；外部服务的成功、原子性或回滚；持久物品、金币、Deals、存档或故事变更；随机分布；寄存器、CCR 与栈效果；完整 H1/ROM 指令一致性；VInt、VDP、DMA、淡入、音乐、文本、窗口、光标或声音行为；畸形列表或状态；以及自然 Map 3 至 Battle 01 可达性。
...
fixture 在固定上游 commit 与已接受 ROM SHA 溯源下通过 H1 解析每个所选符号身份。它不解析并逐字节比较每个函数的完整指令体与 H1 与 ROM。完整源体、宏展开指令字节与任何未来 body-一致性比较保持私有可选验证输入，不是 **已确认** 事实或 H4 要求。
...
fixture 记录跨九个源范围的六个所选条目，有 `1,039` 条语句、`231` 个分支站点、`207` 个直接调用站点（指向 `84` 个唯一目标）、`5` 条所选内部调用边、`59` 个全局状态身份、全部 `8` 个具名输入位、`4` 个战斗动作常量与 `4` 个菜单常量。这些是闭合清单与溯源计数器。重制不得复现这些计数、源范围、分支拓扑或调用站点总计。
...
10. 从通过条件中排除源计数器、原始地址、寄存器/CCR/栈状态、完整 body 一致性、自然 Map 3 或 Battle 01 可达性，以及所有可见/可听/时序主张。
...
| 主张 | 证据 | 判断 |
| --- | --- | --- |
| 十五个入口/表身份与地址 | `function` 加 H1 解析 | 已确认 static identity |
| 单回合、Kiwi、退出、加载与移动命令事实 | `expected.functionFacts` 与固定源检查 | 已确认静态路线/局部结果/请求顺序 |
| 光标、目标、菜单、物品、装备与宝箱事实 | `expected.playerControlFacts` 与固定源检查 | 已确认静态路线/局部结果/请求顺序 |
| 摘要计数器与源成员 | fixture 审计字段 | 仅已确认清单/溯源；不是 H4 行为 |
| 五条窄记录 | 仅地址/清单/调用边面 | 已确认 static identity only |
| pulsating/visual/menu/tactical 用途 | 源码名与注释 | 推断 |
| 输入时序、被调方完成、持久性、AI/目标合法性、呈现、自然场景可达性 | 未被所选所有者建立 | 未知 or separate owner |
