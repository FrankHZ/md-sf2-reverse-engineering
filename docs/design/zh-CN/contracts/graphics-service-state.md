...
- **未知原版行为：** 渲染帧 parity、可见 flash 时长、VInt 或 CRAM-DMA 节奏、硬件时序、最终调色板外观、特殊精灵帧呈现、刻意畸形/调试/原始 RAM 输入的调用方可视行为，以及名义未使用辅助的运行时含义。
...
2. 已接受显示初始化顺序，而不声称可见帧或硬件时序 parity；
...
6. 初始特殊精灵加载与动画刷新保留立即对排队转移接缝的不同，而 DMA/VInt 节奏与可见呈现保持 parity 之外；
...
H4 不要求 Mega Drive VDP 模拟器、指令相同解压器、运行时原版资源格式或帧周期 parity，除非后续显式硬件保真决定添加那些要求。
