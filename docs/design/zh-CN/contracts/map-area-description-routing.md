...
- 源/H1/ROM parity 需要的原版私有字节与 hash。
...
**已确认静态调用图边界：** 已接受 `code/**` 源语料中恰好有一个组装调用 `j_RunMapSetupAreaDescription`。普通探索路径在进入 `CheckArea` 前设置 `d6=1`；`CheckArea` 到达该唯一调用。因为消费者在 `d6` 非零时跳过非零条件字节，所以那三条物理条件函数行——以及对其的全部四个配置扩展引用——在该唯一源码形状路径上被跳过。
...
- source/H1/ROM parity 与公开元数据/私有内容分离。
...
| 区域描述用途 | **仅从源分类法推断** | 源符号/宏 | 玩家面向与叙事含义保持未声称 |
