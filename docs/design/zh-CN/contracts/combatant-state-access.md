...
> 本文件是 [`combatant-state-access.md`](../../contracts/combatant-state-access.md) 的中文镜像。英文原文始终是审阅基线；本镜像为派生文档，遵循 [`glossary.md`](../../glossary.md) 的术语规则（R1–R7）。证据标签按 R1 使用固定中文译法；源码标识符、fixture ID 与路径按 R2 原样保留。
...
| 选择器至少 `128`、至多 `160`（按源码 `bhi` 比较） | 敌人路线 | 减 `96`、掩蔽，然后乘以 56 |
...
这是逻辑一致性模型，不是必需引擎内存布局。原始 56 字节条目与类型化视图共存，使别名偏移、打包字段、带符号坐标读取与源特定变更顺序保持可诊断。应用代码可以暴露更安全类型化状态，但兼容层不得擦除源准入与应用验证之间的区分。
