...
物理潜在伤害是 `max(当前 ATT - 当前 DEF, 1)`，然后按地形设置乘并向下取整：`256/256`、`230/256` 或 `205/256`。在 terrain 之前应用最小值意味着一分估计可以变成零。法术打分从定义威力开始，只应用抗性设置：minor 减四分之一、major 减半、weakness 加四分之一。区域法术优先级按每个受影响目标求和。
