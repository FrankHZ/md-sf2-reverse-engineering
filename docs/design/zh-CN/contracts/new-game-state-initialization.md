...
fixture 在十进制地址 `38710` 绑定 `NewGame`，并拥有恰好七个 `expected.statsFacts.newGame` 事实。对固定源的只读审计确认相同受限形状：设置调用先于己方循环；起始金币分配先于 Bowie 加入；己方初始化器包含已接受空法术赋值与已接受职业/初始/派生属性阶段顺序；设置例程包含三个已接受清除目标加消息速度 `2`。
...
这是逻辑 parity 模型，不是必需引擎内存布局。模型刻意省略己方计数/顺序、原始内容、数字起始金币、数字 Nothing 编码、未接受设置字段、返回值与持久性行为。
...
原版名称与其他版权内容对公开 parity fixture 不必要。公开测试应使用结构元数据、源标识符与合成值。
