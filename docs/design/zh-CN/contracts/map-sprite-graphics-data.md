...
- **已确认原版结构：** 有序 720 槽普通地图精灵指针表、670 个源载荷身份、50 个指针别名、带固定 576 字节解码形式的 669 个有效 Basic 压缩流、共享双字节哨兵身份及其九个指针槽、精确表/载荷一致性，以及下文描述的受限普通对特殊消费者接缝。
...
> 本文件是 [`map-sprite-graphics-data.md`](../../contracts/map-sprite-graphics-data.md) 的中文镜像。英文原文始终是审阅基线；本镜像为派生文档，遵循 [`glossary.md`](../../glossary.md) 的术语规则（R1–R7）。证据标签按 R1 使用固定中文译法；源码标识符、fixture ID 与路径按 R2 原样保留。

## 合同边界

本合同定义普通原版地图精灵图形语料的静态存储、解码、别名、一致性与私有导入边界：
...
受追踪 fixture 暴露聚合计数、地址、一致性、受限解码器诊断与小型哨兵结构。原始压缩字节、解码美术、逐资源 hash 与大小、完整 720 槽别名图与渲染捕获保持私有/生成。
...
全部 720 个指针表条目与全部 670 个源载荷范围匹配已接受 source、H1 与 ROM 边界。指针一致性 `720` 与载荷一致性 `670` 保持独立结果。
...
私有导入保留每个压缩流、解码结果、源路径/地址与私有 hash。公开投影只保留维度、聚合字节计数、诊断、一致性与溯源。两个投影都不从字节形状单独提升瓦片布局、调色板、透明度、动画或可见精灵含义。
...
公开投影不得包含普通压缩载荷、解码精灵美术、逐资源 hash 或大小、完整 720 槽别名图、渲染捕获或其他原版内容。公开报告可以保留受限元数据、聚合计数、一致性、地址、解码器诊断、小型哨兵结构与溯源。
...
- 一个私有 `Mapsprite237_0` `0xFFFF` 哨兵身份，由槽 `711..719` 选择；
- 完整 720 指针与 670 载荷一致性；
...
6. 验证完整 720 指针与 670 载荷一致性，同时保持原版资源、hash、大小与别名私有；
...
| 合同面 | 证据标签 | 精确所有者 | 保留边界 |
| --- | --- | --- | --- |
| 指针与载荷语料 | **已确认静态** | `sf2-map-sprite-decode-v1`；[fixture](../../../../tests/fixtures/h2/map-sprite-decode-v1.json) | 720 私有槽、670 所有者、50 别名、精确一致性；资源/hash 保持私有 |
