...
这是逻辑一致性模型，不是必需引擎循环、内存布局或线程模型。模型刻意把交接身份与优先级和下游效果分开存储。`invokeBattleLoop` 不是战斗结果，`FieldMenu` 不是 UI 实现，地图事件目标名不是完整处理器合同。
...
公开一致性 fixture 只需要结构元数据、身份、码与合成状态。原版地图、事件、物品、对话、图形与音频载荷保持私有/生成输入。
