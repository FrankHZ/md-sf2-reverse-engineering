...
- 已接受证据基线：`79a106f91c2765064ec094402c87492299368e04`；原版发现的精确溯源保留在链接的所有者与测试夹具中。
...
下方证据矩阵保留的旧 8C 捕获、原始时序及硬件未知项是研究边界，不再是完成门槛，也不表示已闭合。旧原版回放路径停用，其账本恢复、runtime capability、scenario transport 与完整硬件 API 都不是里程碑前置条件。main-gate 在里程碑 Issue 处置该路径；仅在具体移除/替代实现有必要时拆子 Issue。停用回放的失败与启动禁令保留。必要原版观察须针对明确玩法问题并复用已接受证据。用户授权的稳定化由已接受 ADR 0015 分段采集修订治理；历史累计上限及反复逐次审批停止条件不能覆盖该授权。
...
产品选择槽与 battle-functions 合同现已闭合。已接受原版证据现已把受控 R1 经 messenger、Map19 位移、royal/guard 完成、自然城堡/塔路线及 Battle 01 准入/生命周期连接至实际首个行动者 2/player-ready。这是已验证原版 core/observer 连续性的 savestate-linked 采集，不是自然 New/读取或不中断墙钟执行。旧 R2d 行动者 1 显式 bridge 保留原边界。判断仍为 **未就绪**：原版战斗动作/RNG/结果、胜利、战后程序及可控 5B 返回、剩余必需 8D 呈现消费/私有 7C、连续合同和适用 H4 定义/执行仍开放。
...
保留的 R2d 审阅复用已接受 #303，不是新的模拟器观察。该审阅检查了[准入研究所有者](../../../research/map3-battle01-admission.md)、[H3 fixture](../../../../tests/fixtures/h3/map3-battle01-player-ready-v1.json)、[fixture schema](../../../../schemas/h3/map3-battle01-player-ready-fixture.schema.json)及[校验器](../../../../src/sf2tool/h3/map3_battle01_player_ready.py)。所属复现命令是 `uv run sf2 h2 map3-battle01-admission` 与 `uv run sf2 h3 map3-battle01-player-ready`；本 docs-only 核对不复跑它们。fixture 显式记录 `explicit-controlled-harness-bridge`、`naturalR2bContinuity: false`，并停在 `ControlBattleEntity.after-WaitForVInt-before-input-read`。因此接受有界就绪状态，但该 fixture 不证明自然 R2a 至 R2b 连续性。下方独立观察闭合从受控 R1 至首次 Battle01 player-ready 的自然路线。

### 已接受的受控 R1 至首次 player-ready 采集

**已确认有界原版观察：**本次核对消费已接受 `79a106f91c2765064ec094402c87492299368e04` 上的[最终采集结果与溯源](../../../research/map3-messenger-acceptance.md#accepted-first-battle01-player-ready-acquisition)、[审计处置](../../../research/map3-battle01-audit.md#proposed-ceilings-and-decision-sequence)及[分段采集修订](../../../decisions/0015-original-reference-replay-and-h4-boundary.md#savestate-linked-segmented-acquisition-amendment)，包括所链接的 main-gate 独立验收。这些所有者保留精确源码/ROM/工具与父级身份、实际交付输入、检查点顺序、消耗、私有配对身份及只读复现方式。此处没有改变 fixture ID 或 R1/R2/R2a/R2d 投影。

兼容的五段链从受控 R1 经 messenger 闭合、Map19 移动至 `(26,29)`、royal/guard 完成与已验证保存/加载，再经 Maps 21/40/57、CheckBattle、BattleLoop、before/load/start 完成、activation/region/spawn、generation 及首个分发。实际首个行动者为 **2**，区别于 seeded R2d 行动者 1。在 `ControlBattleEntity` 的 `0x22E70`，`WaitForVInt` 之后、读取输入之前，窗口计数 2 与调色板模式 5 是合法非阻塞战场呈现；实际模态/输入/目标/动作/事件/滚动 guard 清除，且无阻塞消费者待处理。这只接受该输入就绪边界，不证明所有呈现提示。

回调为 observer/emulator 帧 **22368/22367**；随后独立完成的中立保存帧为 **22368/22368**、PC `0xF00`。最终配对仅作证据，`resumable=false`，不是延续父级。就绪后没有战斗动作或进一步玩法输入。五段均以已验证父级连续性及清理完成。尝试 21 缺失的最终 observer 恢复及回调移除载荷仍为**未知**；成功不能修复它。

[旧 Map19-only 观察](../../../research/map3-messenger-acceptance.md#single-admitted-interactive-acquisition-result)保留其自身 `(26,30)` 位移前边界、仅适用该次尝试的零盟友状态字、`OBSERVATION-COMPLETE-UNREVIEWED` 原始标签及独立验收。其 post-447 等待精确入口仍为**推断**。这些是历史局部限制，不是当前自然路线前沿。既有失败及实际消耗均保留于 Research 所有者；稳定化取代旧累计尝试/时间/帧/批次退役与逐次审批停止条件。原生菜单恢复、未观察分支及完整战斗/5B/8D/H4 仍为**未知**。本文件不增加运行时、冻结回放、准备或私有输入修改。
...
[战斗控制与战斗员生命周期](../../contracts/battle-control-lifecycle.md) 闭合通用胜利变更顺序：治愈合格队伍、运行战后接缝、清除解锁标志、设置完成标志并返回 `D4=1`。较早 Battle 01 调试 H3 入口使用 Debug Battle Test 并跳过开战/开战前过场。下方独立 R2d 显式 bridge 观察确实观察了这些程序返回，但两种入口均不建立自然 Battle 01 准入。独立的受控 R1 分段采集现已建立该准入与首次就绪；原版战后效果及里程碑端点仍开放。

### 已接受场景证据链刷新

已接受的场景专用所有者现形成以下精确有界链。运行时与静态标签不可互换。每行保留边界描述其自己的 fixture 或观察，并不否定独立 R2d、Map19-only 或受控 R1 至首次 player-ready 结果：
...
| 受控 R1 至首次 Battle01 player-ready（独立分段采集） | [已接受结果/溯源](../../../research/map3-messenger-acceptance.md#accepted-first-battle01-player-ready-acquisition)；[审计处置](../../../research/map3-battle01-audit.md#proposed-ceilings-and-decision-sequence) | **已确认** Map19 位移、royal/guard 完成、自然城堡/塔路线及战斗准入/生命周期，至实际行动者 2 在 `0x22E70` 就绪 | 受控起点；回调与完成帧不同；最终配对不可恢复；不证明动作/胜利/5B、不中断墙钟、冻结回放或完整 8D/H4；旧 fixture 不变 |
...
| --- | --- | --- | --- |
| 准入的 Map 3 起点 | [新游戏状态初始化](../../contracts/new-game-state-initialization.md)、[存档系统](../../contracts/save-system.md)、[故事推进](../../synthesis/story-progression.md)、[Map 3 受控准入](../../contracts/map3-controlled-admission.md)、R1 所有者 | **合同就绪 / 有界运行时**，闭合受控准入快照至首个探索等待；**已接受产品决定** 选择 1A | 自然 New/读取等价性仍为**未知**；独立分段采集拥有后续路线；8C 与原始时序不属于验收域 |
| Map 3 配置与内容 | [地图配置数据](../../contracts/map-setup-data.md)、[地图与探索](../../contracts/map-exploration.md)、上方 R1/R2/R2a、独立分段采集与 R2b 所有者 | 所选默认配置、到达前缀及合法延续拓扑具有**合同就绪静态/有界运行时所有者**；连续场景组合仍**缺失设计合同** | 至首次 player-ready 的路线已观察；后续效果、路线所需完整内容/连续合同保持**未知/推迟**；不批量关联 26 行 |
| 探索循环与输入 | [探索控制流](../../contracts/exploration-control-flow.md)、[输入系统](../../contracts/input-system.md)、[地图入口路由状态](../../contracts/map-entry-routing-state.md)、上方 R2/R2a 与独立分段采集 | **合同就绪有界运行时前缀**与局部交接；现代逻辑控制/可访问性已有**已接受产品决定** | Map19 位移及至首次 Battle01 就绪的输入时序已观察；就绪后输入/结果时序仍为**未知/推迟** |
| 对话与交互 | [对话系统](../../contracts/dialogue-system.md)、[精灵对话属性数据](../../contracts/sprite-dialogue-property-data.md)、[文本与字体系统](../../contracts/text-and-font-system.md)、[立绘窗口状态](../../contracts/portrait-window-state.md)、上方 R2a 与独立分段采集 | **合同就绪静态接缝与有界 messenger 结果**，另有已接受真实 messenger/gate 文本消费者及返回；私有本地原版文本已有**已接受产品决定** | post-447 等待精确入口仍为**推断**；分段链已观察 royal/guard 及 before/start 完成；剩余必需呈现消费/完成及就绪后连续性仍为**未知/推迟** |
...
| 地图到战斗准入 | [探索控制流](../../contracts/exploration-control-flow.md)、[地图入口路由状态](../../contracts/map-entry-routing-state.md)、[战斗遭遇定义](../../contracts/battle-encounter-definition.md)、[战斗过场路由](../../contracts/battle-cutscene-routing.md)、上方 R2b/R2c/R2d 与独立分段采集所有者 | **合同就绪静态路线/准入主干及已确认的受控 R1 自然准入**；连续场景组合仍**缺失设计合同** | 至首次 player-ready 的自然路线/准入已观察；连续合同、就绪后动作及更广调用方状态泛化仍开放；R2d 保留其显式 bridge |
| Battle 01 遭遇配置 | [战斗遭遇定义](../../contracts/battle-encounter-definition.md)、[战斗控制与战斗员生命周期](../../contracts/battle-control-lifecycle.md)、[战场导航](../../contracts/battlefield-navigation.md)、上方 R2c/R2d/R3a 与独立分段采集所有者 | **合同就绪静态遭遇/控制主干及有界初始化/玩家就绪观察** | 受控 R1 自然生命周期及实际首个行动者 2 已观察；接缝后输入、后续回合状态及其他初始状态仍为**未知/推迟** |
| 玩家回合与战斗菜单 | [战斗函数控制流](../../contracts/battle-functions-control-flow.md)、[输入系统](../../contracts/input-system.md)、上方 R2d/R3a 与独立分段采集所有者 | **合同就绪静态分支/请求/局部输出所有者及有界 R2d 行动者 1 分发及受控 R1 实际行动者 2 就绪**；手动能动性与 UI 已有**已接受产品决定** | 就绪接缝后输入、AI 执行、命令、移动、目标、动作、取消与结果仍为**未知/推迟** |
...
| 端到端 H4 | 所有具名子系统测试夹具与合同 | 台账**综合就绪**；已接受静态链与产品层存在；首次 player-ready 之后的原版参考运行时依 ADR 0014–0016 为**未知/推迟**；场景组合与可执行定义仍**缺失设计合同** | 添加一个消费而非削弱子系统 fixture 的证据绑定连续场景合同；失败的原版参考候选保持非证据 |
...
| 路线 | **已接受：2A 最小研究证明自然路线** | 分段采集观察受控 R1 经 Map19 位移及城堡/塔完成至首次 player-ready 的路线。完整路线必需内容与下游效果仍需证据绑定组合；不泛化至替代路线。 |
| 自然战斗/过场 | **已接受：3A 时间顺序，占位子条款由 7C/8D 取代** | 分段采集确认自然准入、before/load/start 及初始化至实际行动者 2/player-ready。剩余必需呈现消费及更广状态主张仍开放；R2d 保留 seeded bridge。 |
...
| 视觉/音频一致性 | **已接受：8D 呈现语义** | 已接受 messenger/royal/guard 与战斗生命周期完成及首次 player-ready guards 可复用；剩余到达场景/资源/提示身份、语义顺序/完成/就绪与 H4 定义仍开放；排除硬件精确性 |
...
| RA-02–RA-05 / 路线、准入、状态 | 受控 R1 分段自然路线、Map19 位移、royal/guard 完成、战斗生命周期及首个行动者 2/player-ready；旧 fixture 不变 | 保留至 `0x22E70` 的有界闭合及独立完成帧。剩余运行时前沿从就绪后开始；连续合同及任何额外主张的 caller/accounting/state 字段须有已接受溯源。 |
...
| RA-08 / RA-09 / 菜单与对话 | R2/R2a 的 `FieldMenu` NotReached；独立采集没有 FieldMenu 回调，且有真实 messenger/gate 文本返回；分段采集增加 royal/guard 及 before/start 完成 | 复用这些有界结果；post-447 等待精确入口仍为推断。只闭合剩余路线必需覆盖。不建立全面野外菜单或可选对话任务；正文/捕获保持私有。 |
| RA-10 / 持久性 | 已接受的 6A 排除 | 本里程碑不需要存档实现或持久性观察。H4 必须检查重启回到准入状态，以及不存在用户存档/恢复界面。 |
| RA-11 / 原版呈现 | 静态呈现/资源所有者及已接受 messenger/royal/guard 消费者、战斗生命周期及首次 player-ready guards | 复用已观察返回/就绪，不将其当作全部视觉/音频完成。剩余必要 8D 身份/顺序、完成、就绪及溯源仍开放。素材可用本身不证明呈现消费。旧硬件缺口不是里程碑要求。 |
...
**未知原版保真 / 显式实现限制：**[能力所有者](../../../../remake/docs/capability-status.md#milestone-and-deferred-surfaces)保留剩余 caller/accounting/seed/egress 比较、完整呈现及 H4 缺口。EGRESS 及更广动作、状态、物品/配置事件等未支持消费者，只有被接受路线/动作集需要时才成为工作项。现代 white fades、mosaic、shiver 及项目自有提示音是已实现的呈现服务；其存在本身不建立原版提示身份、语义顺序、完成或 7C 合规。应比较这些 8D 边界。原版像素/帧/音频波形差异不在选定域内，不要求精确后端或硬件捕获；不能把影响玩法的差异或私有原版内容要求当成视觉润色排除。

**独立 7C 音频缺口：**[Research 呈现审计](../../../research/map3-battle01-audit.md#presentation-sufficiency-under-8d)及[重制呈现所有者](../../../../remake/docs/exploration-programs.md)说明 `MUSIC_JOIN`/`MUSIC_SAD_JOIN` 是项目自编和弦循环。原版命令 ID 与实际 `AudioStreamPlayer` 启动/淡入淡出/停止不建立私有原版素材身份/溯源及其到已消费提示的绑定。已接受 first-player-ready 采集与 8D 语义比较不关闭该内容缺口；9A/10A 也不豁免它。
...
| 原版行为证据 — Research | 已接受受控 R1 至首次 player-ready 路线；具体未决玩法主张 | 以已接受方法和声明溯源建立战斗输入/动作/RNG/结果、胜利、战后程序及可控 5B。最终 player-ready 配对不可恢复。保留真实故障与失败，不依赖停用回放。 |
...
[已接受分段采集修订](../../../decisions/0015-original-reference-replay-and-h4-boundary.md#savestate-linked-segmented-acquisition-amendment)记录用户要求持续稳定化，取代旧累计尝试/时间/帧/批次上限、失败退役及反复逐次审批停止条件。历史尝试及全部实际消耗仍计入所有者，包括 #460/#465 及失败尝试 21 的清理**未知**。已验证 core/observer/source 身份、兼容父级加载、I/O 与不可恢复回调失败、单步/exchange/断连隔离及 main-gate 审阅仍具约束力。普通未就绪输入/保存请求是暂停的零帧结果，不自动触发失败尝试退役。该授权不重开停用冻结回放，也不使仅作证据的最终配对可恢复。下个 Research outcome 为 [#496](https://github.com/FrankHZ/md-sf2-reverse-engineering/issues/496)；其未合并实现/结果不是本文证据。本台账不虚构下游值或完整连续合同。
...
已记录候选 preflight `B003732B61A375C7980BDC1F328E5C8B00553E6F38E669746041E9E6BC7BE0EA` 返回 PASS、`ProcessStarts=0`。这是保留的 preflight 结果，不是启动收据或 runtime 兼容性证明，本 docs-only 修正也不复跑它。不得假定任何 ordinal 可用。Ordinal 3 仍要求单一同候选 ordinal-2 PASS、实际收据 hash 校验及匹配 replay digest；恢复一份报告为 ordinal-2 FAIL 的收据不能满足该条件。没有授权额外诊断、第四次启动或通过任务/wrapper/scenario/transport 重置。这些停用回放限制不覆盖上方独立接受的分段采集授权。

### 有条件运行时问题

以下是具体验收缺口的准入材料，不是运行授权。ADR 0014/0016 要求在创建任何新 H3 fixture 前证明调用方依赖、实质合同影响，以及既有批量轨道无法覆盖。优先扩展已接受轨道；不按 NPC 或每个 Unknown 建任务。此处适用上方 ADR 0015 方法区分：只有冻结回放要求非自适应输入轨迹；交互式采集允许明确、基于观察的控制器选择。两者均保留私有输出及类型化回调/退出/清理失败。当前采集遵循已接受修订；历史累计上限不再作为新门禁。

| 问题与验收影响 | 要复用的既有静态证据/轨道 | 独立准入与停止边界 |
| --- | --- | --- |
| 新行为观察是否必要且可准入？ | 先复用已接受事实/rails；声明缺失玩法断言及必要检查点。停用 transport 不是前置。 | 适用 ADR 0014/0015/0016 与既有用户授权，保留 source/core/identity/I/O/observer 故障隔离及 main-gate 审阅；不恢复已取代的逐次审批停止条件。 |
| 首次 player-ready 之后，哪些原版输入/动作/RNG/结果到达胜利、战后程序及可控 5B？这决定剩余 4A/5B 与 RA-06/RA-07/RA-12。 | 已接受受控 R1 自然准入及行动者 2 就绪；R3a–R3d/R4a 静态所有者与局部战斗合同。最终配对不可恢复；R4a 在探索执行前停止。 | Research 在当前授权下拥有兼容起点/延续方法及缺失字段。保留受控起点溯源、禁止实时状态注入并保留真实故障隔离。只有独立接受的下游结果能推进本台账。 |
| 8D 需要哪些到达呈现提示、顺序、完成及输入就绪状态？这决定 RA-11 与 H4 第 9 层。 | 复用已接受 messenger/royal/guard 返回、战斗生命周期及首次 player-ready guards，以及对话/窗口、图形/音频/资源/程序所有者。post-447 等待精确入口仍为推断；仅 ID/返回不能建立后续提示及实际 host 消费。 | 命名已到达提示/资源、调用方、分发/消费者完成或确认、关联状态效果及输入阻塞/可用关系。持续音乐需要到达处的启动/替换/停止语义，不是虚构结束事件。先复用证据；只有实质未决断言才能支持单独准入观察。硬件 API/时钟不是前置；7C 素材溯源仍独立。 |
...
| 受控 R1 经城堡/塔至首次 player-ready 的自然路线连续性 | PASS（有界） | [已接受分段采集](../../../research/map3-messenger-acceptance.md#accepted-first-battle01-player-ready-acquisition)；已验证保存/加载连续性，不主张不中断墙钟或就绪后行为 |
| 所需探索/对话/菜单/UI 范围精确 | 开放 | 研究加路线所需条件合同；ADR 0010 固定最小范围规则 |
| 静态 Battle 01 准入主干已接受 | PASS | `sf2-map3-battle01-admission-static-v1`；自然准入由下方独立有界 PASS 表示 |
| 显式 bridge Battle 01 初始化与 PlayerReady 接缝 | PASS（有界） | `sf2-map3-battle01-player-ready-runtime-v1`；受控 bridge，不证明自然连续性或接缝后游玩 |
| 受控 R1 自然 Battle 01 准入至首次 player-ready | PASS（有界） | 已接受分段采集：实际行动者 2 在 `0x22E70` 就绪；回调与完成帧不同，最终配对不可恢复；连续合同仍开放 |
...
| 必要自然行为参考证据已接受 | 首次 player-ready 之后开放 | Research；战斗动作/RNG/结果、胜利/战后程序/5B 及剩余必需 8D；已接受稳定化授权取代历史累计上限 |
...
fixture 行保留各自限制；独立 Map19 与 first-player-ready 采集不更新其投影。
...
| 受控 R1 经已验证 savestate-linked 城堡/塔连续性自然到达 Battle01 首个行动者 2/player-ready | **已确认有界原版观察** | [已接受最终结果/溯源](../../../research/map3-messenger-acceptance.md#accepted-first-battle01-player-ready-acquisition)及[审计处置](../../../research/map3-battle01-audit.md#proposed-ceilings-and-decision-sequence) | 回调不同于完成帧；不可恢复的仅证据配对；就绪后动作/RNG/结果、胜利/5B、完整 8D/7C/H4 及 failed21 清理仍开放；不主张不中断墙钟或冻结回放 |
