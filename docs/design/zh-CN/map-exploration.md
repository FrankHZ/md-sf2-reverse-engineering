...
图块集是有序的 3x3 格子字记录数组。原版图块索引 0-2 是加载器在压缩命令之前构建的内置空宝箱、闭宝箱与开宝箱图块。导入管线必须在解码之后把这些暴露为普通索引图块，这样事件与碰撞逻辑就不需要特殊的负值或带外同一性。

现代渲染器只有在每个位的含义被确认之后，才可以规范化格子与图块标志为命名字段。在此之前，canonical import 保留原始 16 位字与任何被证明的解读并列。渲染便利不得破坏未知位或重写源码证据。
