...
H2、成长曲线与法术学习合同，以及 RNG/成长计算/行动顺序/物理基础伤害整机运行时 H3；尚未下载外部补丁、选择现代重制引擎或开始
重制实现。
...
固定 BizHawk/Genesis Plus GX 的 RNG、成长计算、行动顺序与物理基础伤害 H3；后续在同一入口继续补齐：
...
dead/unplaced 过滤与 signed-byte 稳定排序；AI 驱动的自然攻击另确认 30% 地形减伤和弓手对 hovering
目标的 25% 加成顺序及整数截断。
...
下一块继续扩展 **Phase 2 的运行时证据**：观察 Battle 01 跨越 activation region 的敌人状态
变化，并把已确认的基础伤害继续推进到随机 spread、critical/double/counter 与 HP/EXP 应用。
