...
global label 和 116 个直接调用点，并将每个文件的代表入口绑定到 H1 地址；当前只确认 inventory
边界，移动数组、范围传播与 move-string 语义按后续静态批次逐步加深。H3 以 7 组受控 seed 验证
