...
目标的 25% 加成顺序及整数截断，并继续覆盖受控 critical、两次向下 spread、HP 零下限、击杀
判定和单行动 49 EXP 累加上限。
...
下一块继续扩展 **Phase 2 的运行时证据**：把已确认的伤害脚本构造结果推进到 battle-scene
reaction 的持久 HP/EXP 回放，并补齐 dodge/double/counter 分支。
