...
H2、成长曲线与法术学习合同、battle AI 全目录 inventory、五类 action filter、攻击/治疗/支援决策静态合同，以及基础/调试覆盖 RNG、成长计算/完整升级（含投影后成长、
...
截至 2026-07-18，研究索引有 85 条 confirmed finding、52 个 H3 fixture 和 441 个地址绑定。
按固定上游的 387 个 `disasm/code` ASM 文件作严格分母，已有可执行证据触达 40 个文件，即
**10.34% code-file reach**；这不是行/函数覆盖率，也不表示这些文件已全部理解。H2 的 14 个 ROM
...
movetype/AOE scoring，以及 support admission、DISPEL/MUDDLE 2 target scoring 和两个不可达
ATTACK/BOOST 2 路由，并将 30 个新 symbol 绑定到 H1 地址。H3 以 7 组受控 seed 验证 `GenerateRandomNumber` 的原版 ROM 指令、RAM seed
