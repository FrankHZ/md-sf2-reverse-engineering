...
H2、成长曲线与法术学习合同、battle AI、battlefield、battle-loop、顶层 battle control、battle actions、shared battle functions、battle scene 根引擎/动画实现、battle cutscene、common scripting 与 common maps 全目录 inventory 和静态决策/生命周期合同，以及基础/调试覆盖 RNG、成长计算/完整升级（含投影后成长、
...
截至 2026-07-19，研究索引有 301 条 confirmed finding、54 个 H3 fixture 和 697 个地址绑定。
按固定上游的 387 个 `disasm/code` ASM 文件作严格分母，已有可执行证据触达 225 个文件，即
**58.14% code-file reach**；这不是行/函数覆盖率，也不表示这些文件已全部理解。H2 的 14 个 ROM
