...
H2、成长曲线与法术学习合同，以及 RNG/成长计算/行动顺序/区域激活/物理基础伤害整机运行时 H3；尚未下载外部补丁、选择现代重制引擎或开始
重制实现。
...
固定 BizHawk/Genesis Plus GX 的 RNG、成长计算、行动顺序、区域激活与物理基础伤害 H3；后续在同一入口继续补齐：
...
下一块继续扩展 **Phase 2 的运行时证据**：把已确认的基础伤害继续推进到随机 spread、
critical/double/counter 与 HP/EXP 应用。
