...
BLAZE 2 四档 FIRE 抗性矩阵、DAO 四目标 power division 的整机运行时 H3；尚未下载外部补丁、选择现代重制引擎或开始
...
固定 BizHawk/Genesis Plus GX 的基础/调试覆盖 RNG、成长计算/完整升级及投影/上限/法术继承边界、战斗 EXP 自然升级、行动顺序、区域激活、物理伤害计算链与首个法术抗性矩阵 H3；后续在同一入口继续补齐：
